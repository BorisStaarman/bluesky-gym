...
STEP_PENALTY = -0.01
INTRUSION_PENALTY = -6.0  # Separation violation - episode terminates
WAYPOINT_REACHED_REWARD = 2  # Increased to encourage completion
# waypoint rewards
...
PROXIMITY_MAX_PENALTY = 0.6 * abs(INTRUSION_PENALTY)  # -2.0 when INTRUSION_PENALTY is -2.5
...
METRICS_BASE_DIR = "metrics_24_10"
...
class SectorEnv(MultiAgentEnv):
    ...
    def __init__(self, render_mode=None, n_agents=10, run_id="default",
        ...
        self.proximity_max_penalty = 0.6 * abs(self.intrusion_penalty)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # for evaluation
        self.total_intrusions = 0
        self._intrusion_this_step = False  # Track intrusion detection for episode termination
    ...
    def step(self, actions):
        ...
        for a in agents_to_remove:
            ...
            intrusion_termination = self._intrusion_this_step if hasattr(self, '_intrusion_this_step') else False
            ...
            self._agent_buffers[a].append({
                ...
                "terminated_intrusion": intrusion_termination,  # True if episode ended due to intrusion
        ...
        with open(path, "a", newline="") as f:
            w = csv.DictWriter(
                ...
                fieldnames=[
                    ...
                    "total_intrusions",
                    "terminated_intrusion",  # True if episode ended due to intrusion
    ...
    def _get_reward(self, active_agents):
        ...
        intrusion_detected = False  # Track if ANY intrusion occurred this step
        ...
        # Store intrusion flag for use in _get_terminateds
        self._intrusion_this_step = intrusion_detected
    ...
    def _get_terminateds(self, active_agents):
        # If intrusion detected this step, terminate ALL agents immediately
        if hasattr(self, '_intrusion_this_step') and self._intrusion_this_step:
            return {agent: True for agent in active_agents}
        
        # Otherwise, only terminate agents that reached their waypoint
    ...
    def _check_intrusion(self, agent_id, ac_idx):
        ...
        Returns:
            tuple: (step_penalty, intrusion_occurred)
                - step_penalty: float, the penalty for intrusions
                - intrusion_occurred: bool, True if any intrusion was detected (episode terminates)
        """
        had_conflict = False
        step_penalty = 0.0
        ...
        return step_penalty, had_conflict
