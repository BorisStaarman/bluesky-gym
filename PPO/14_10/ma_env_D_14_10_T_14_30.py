...
class SectorEnv(MultiAgentEnv):
    ...
    def __init__(self, render_mode=None, n_agents=10):
        ...
        # Define the space for a single agent
        ...
        # --- CRITICAL FIX: Use spaces.Dict for multi-agent spaces ---
        ...
        # Logging and rendering attributes
        self.window = None
        self.clock = None
        
        # dictionary to store agent waypoints
        ...
        self.step_reward_drift = 0.0
        self.step_reward_intrusion = 0.0
        self.step_reward_progress = 0.0
    ...
    def reset(self, *, seed=None, options=None):
        ...
        for agent in self.agents:
            try:
                ...
                wpt_lat, wpt_lon = self.wpts_latlon[ac_idx]
    ...
    def step(self, actions):
        ...
        # --- MODIFICATION: The _get_reward method will now also return infos ---
        observations = self._get_observation(agents_in_step)
        rewards, infos = self._get_reward(agents_in_step) # Capture infos here
        terminateds = self._get_terminateds(agents_in_step)
        truncateds = self._get_truncateds(agents_in_step)
        
        agents_to_remove = {
            agent for agent in agents_in_step 
        ...
        # Return the infos dictionary populated by _get_reward
    ...
    def _get_reward(self, active_agents):
        ...
        for agent in active_agents:
            try:
                ...
                self.step_reward_drift += drift_reward
                self.step_reward_intrusion += intrusion_reward
                self.step_reward_progress += progress_reward

                rewards[agent] = drift_reward + intrusion_reward + progress_reward
                
                # --- MODIFICATION: Populate the info dict for this agent ---
    ...
    # ... The rest of your file (_render_frame, _get_observation, etc.) can remain the same ...
    # Make sure all other methods are correctly indented within the SectorEnv class.
    
    # Placeholder for the rest of your class methods
    ...
    def _generate_waypoints(self):
        ...
        for d in d_list:
            while edge_idx < len(edges) - 1 and d > current_d + edges[edge_idx][2]: current_d += edges[edge_idx][2]; edge_idx += 1
            edge, frac = edges[edge_idx], (d - current_d) / edges[edge_idx][2]; self.wpts.append(edge[0] + frac * (edge[1] - edge[0]))
        self.wpts_latlon = [fn.nm_to_latlong(CENTER, p) for p in self.wpts]
    
    def _generate_ac(self):
        min_x, max_x = self.poly_points[:, 0].min(), self.poly_points[:, 0].max()
        min_y, max_y = self.poly_points[:, 1].min(), self.poly_points[:, 1].max()
        init_p_latlong = []
        ...
        for agent, idx in zip(self._agent_ids, range(self.num_ac)): 
            wpt_agent = self.wpts_latlon[idx]
