...
PRETRAIN_EPISODES = 100  # Increased from 150 for better state-action coverage
...
BURN_IN_TEMPERATURE_LR_LOCAL = 1e-3  # Match PPO: unified LR for temperature
BURN_IN_ACTOR_LR_LOCAL = 1e-3        # INCREASED from 7.5e-7 to match PPO unified LR
BURN_IN_ATTENTION_LR_LOCAL = 1e-3    # Match PPO: unified LR for attention
BURN_IN_CRITIC_LR_LOCAL = 1e-3       # Match PPO: unified LR for critic
...
class ForceAlphaCallback(DefaultCallbacks):
    ...
    def on_train_result(self, *, algorithm, result, **kwargs):
        ...
        # ========================================
        # Gradually increase intrusion penalty severity through training stages
        ...
        # Gebruik de moving average (smoothed) waypoint rate voor stabiliteit
        ...
        old_penalty = CURRENT_INTRUSION_PENALTY
        target_penalty = old_penalty

        # --- 1. DYNAMISCHE PENALTY MET STREAK ---
        ...
        result["custom_metrics"]["intrusion_penalty_current"] = float(CURRENT_INTRUSION_PENALTY)
        result["custom_metrics"]["success_streak"] = float(self.success_streak)
...
def prefill_sac_buffer(algo, n_episodes=30):
    ...
    HEADING_THRESHOLD = 0.1  # Threshold for "active steering" (normalized action space)
    OVERSAMPLE_FACTOR = 4.5   # REDUCED from 2 to 1.5 for better balance
    ...
    cfg = (
        ...
        .environment(
            ...
            env_config={
                ...
                "intrusion_penalty": CURRENT_INTRUSION_PENALTY,  # Pass current penalty to envs
            },
