...
TOTAL_ITERS = 3500    # SCALED: 16000→3500 (stability test run)
...
START_ALPHA = 0.10  # RAISED: 0.05→0.10 (Q overestimation=2.55 dwarfed alpha=0.02; actor went fully greedy, actor loss→-0.4)

# IMPORTNAT PARAMETERS FOR PRETRAINING AND BURN-IN
PRETRAIN_EPISODES = 200       # Keep at 200: more expert data = better Q signal, not scaled down
BURN_IN_ITERATIONS = 1000      # SCALED: 2000→500 (2_25 is 3500-iter run; 2000 burn-in = 57% of training → overfit)
...
EXPERT_MIX_DECAY_UNTIL = 0.25  # Decay to 0% by 25% of total iterations (~875 iters)

# --- Burn-in Phase Parameters (Offline Learning from Expert Buffer) ---
BURN_IN_BATCH_SIZE = 4096     # Batch size for burn-in sampling
ENABLE_BURN_IN = True         # Set to False to skip burn-in phase entirely

# --- Burn-in Checkpoint (skip retraining Stage 1 on subsequent runs) ---
# Workflow:
#   First run  : SKIP_BURN_IN_LOAD_CHECKPOINT = False  → runs burn-in, saves weights to BURN_IN_CHECKPOINT_DIR
#   Later runs : SKIP_BURN_IN_LOAD_CHECKPOINT = True   → restores saved weights, skips burn-in
SKIP_BURN_IN_LOAD_CHECKPOINT = False  # ← flip to True to reuse a completed burn-in

# --- KL Divergence Penalty (Keep Policy Close to Expert During Burn-in) ---
# DISABLED: KL reward shaping is ~1000x larger than env rewards at /12000 scale.
# reward_bonus = -coeff * MSE ≈ -0.2, but env step rewards ≈ 0.00001. Drowns Q-signal.
# The expert buffer data itself provides sufficient imitation signal via critic calibration.
ENABLE_KL_PENALTY = False       # DISABLED: reward shaping incompatible with /12000 scaling
KL_COEFF_START = 2.0          # Unused (kept for reference)
KL_COEFF_END = 0.0
KL_ANNEAL_UNTIL = 2000
...
# CRITICAL FIX for /12000 scaling: rewards are 15x smaller → gradients 15x smaller
# Actor LR must be 15x higher to achieve same effective update as before
BURN_IN_TEMPERATURE_LR_LOCAL = 0.0   # FROZEN for learn_on_batch SAC actor loss (entropy term degrades policy)
BURN_IN_ACTOR_LR_LOCAL = 0.0         # FROZEN for learn_on_batch SAC actor loss (entropy term degrades policy)
BURN_IN_ATTENTION_LR_LOCAL = 0.0     # FROZEN for learn_on_batch SAC actor loss
BURN_IN_CRITIC_LR_LOCAL = 3e-4       # Unchanged: calibrate critic on expert trajectories
# Separate behavioral cloning LRs — used in BC step AFTER learn_on_batch (no entropy interference)
BURN_IN_BC_LR = 1e-4                 # Base actor BC learning rate: MSE(policy_mean, expert_action)
BURN_IN_BC_ATTENTION_LR = 1e-4       # REDUCED: 6e-4→1e-4 (6e-4 caused BC Loss to diverge: attention sharpness oscillated 0.10-0.16, policy drifted FROM expert)
BURN_IN_BC_TEMPERATURE_LR = 1e-4     # REDUCED: 6e-4→1e-4 (match attention — temperature overshooting was co-cause of instability)
...
MAIN_ACTOR_LR_SCHEDULE = [
    ...
    [100, 5e-6],      # Gradual ramp-up  (was 500)
    [500, 2e-5],      # Peak: collision avoidance mastery  (was 2000)
    [1500, 1e-5],     # Maintain for extended learning  (was 6000)
    [2500, 5e-6],     # Final refinement  (was 12000)
]
...
# BOOSTED 3-5x: previous run showed temperature barely moved (2.99→3.42 vs expected 3.0→4.86)
MAIN_ATTENTION_LR_SCHEDULE = [
    [0, 5e-5],        # Protect burn-in calibration
    [1750, 5e-5],     # FROZEN: keep at 5e-5 until expert mix ends (sharpness collapsed 0.19→0.09 when ramp hit ~2e-4 at iter 1500)
    [2500, 1e-4],     # Gentle ramp only after policy has stabilized
    [3200, 5e-5],     # Refinement
]

# Temperature Parameter LR Schedule (for attention sharpness control)
# BOOSTED 3-5x: temperature barely moved in previous run, needs much stronger gradient
MAIN_TEMPERATURE_LR_SCHEDULE = [
    [0, 5e-5],        # Protect burn-in temperature calibration
    [1750, 5e-5],     # FROZEN: match attention (temperature overshoot was co-cause of sharpness oscillation)
    [2500, 1e-4],
    [3200, 5e-5],
]

# Critic LR Schedule (for Q-value learning)
# UPGRADED for /12000 scaling: Higher LRs because Q-values are 15x smaller (easier to learn)
MAIN_CRITIC_LR_SCHEDULE = [
    [0, 3e-4],        # REDUCED: 5e-4→3e-4 (high initial critic LR caused Q overestimation chain)
    [500, 3e-4],      # Hold steady while actor stabilizes
    [1300, 2e-4],     # Reduce as Q-values calibrate
    [2000, 1e-4],     # Stabilization
    [3000, 5e-5],     # Final refinement
]
...
BURN_IN_CHECKPOINT_DIR = os.path.join(script_dir, "models/burn_in_checkpoint")  # Saved after burn-in completes
...
# 4-stage progressive penalty increase to balance waypoint learning + intrusion avoidance
# REVISED: Stronger penalties to enforce collision avoidance throughout training
INTRUSION_CURRICULUM_STAGES = [
    (0, -100),       # Stage 1: Learn navigation
    (1200, -120),    # Stage 2 (was 875): DELAYED past expert mix ramp-down — prev run collapsed when both hit at iter 875
    (2000, -150),    # Stage 3 (was 1750): Moderate penalties
    (2800, -200),    # Stage 4 (was 2500): Full penalties
]
...
class ForceAlphaCallback(DefaultCallbacks):
    ...
    def on_train_result(self, *, algorithm, result, **kwargs):
        ...
        # ========================================
        FREEZE_UNTIL = 500    # SCALED: 2000→500 (same ~14% of total iters)
        DECAY_UNTIL  = 2500   # SCALED: 12000→2500 (~71% of total iters)

        # Define target values - ADJUSTED for /12000 scaling (smaller rewards need less alpha)
        ALPHA_FREEZE = 0.02   # REDUCED: 0.05→0.02 (entropy-reward balance for small rewards)
        ALPHA_FINAL  = 0.02  # REDUCED: 0.015→0.005 (minimal exploration at end)
...
def burn_in_on_expert_buffer(algo, n_iterations=2000, batch_size=4096):
    ...
    # Per-iteration action statistics - key diagnostic for KL effectiveness
    action_std_history = []   # Should DECREASE as policy converges to expert
    action_mean_history = []  # Should converge toward 0 (expert mean is near-zero for MVP)
    ...
    N_EVAL_EPISODES = 10  # INCREASED: 5→10 (5 episodes too noisy, WP rate oscillating 0%→31%)
    
    # Helper function: fixed burn-in actor LR
    def get_actor_lr_for_burnin(iteration):
        return BURN_IN_ACTOR_LR_LOCAL
    
    # --- BC OPTIMIZER: dedicated optimizer for behavioral cloning step ---
    # Created once here (separate from SAC's internal optimizers, avoids policy.optimizers() issues)
    bc_optimizer = None
    if BURN_IN_BC_LR > 0.0:
        try:
            _bc_actor_model = policy.model.action_model if hasattr(policy.model, 'action_model') else policy.model
            _bc_param_groups = []
            for n, p in _bc_actor_model.named_parameters():
                if not p.requires_grad:
                    continue
                if 'temperature' in n.lower():
                    _bc_param_groups.append({'params': [p], 'lr': BURN_IN_BC_TEMPERATURE_LR})
                elif 'attention' in n.lower() or 'w_q' in n.lower() or 'w_k' in n.lower() or 'w_v' in n.lower():
                    _bc_param_groups.append({'params': [p], 'lr': BURN_IN_BC_ATTENTION_LR})
                else:
                    _bc_param_groups.append({'params': [p], 'lr': BURN_IN_BC_LR})
            bc_optimizer = torch.optim.Adam(_bc_param_groups)
            print(f"   ✅ BC optimizer created: base_lr={BURN_IN_BC_LR:.1e}, attn_lr={BURN_IN_BC_ATTENTION_LR:.1e}, temp_lr={BURN_IN_BC_TEMPERATURE_LR:.1e}")
        except Exception as e:
            print(f"   [WARN] BC optimizer creation failed: {e}. BC step will be skipped.")
            bc_optimizer = None
    ...
    for i in range(1, n_iterations + 1):
        ...
        # --- BC STEP: Behavioral Cloning — actor imitates expert actions directly ---
        # SAC actor loss fails on expert buffer: flat Q → entropy term dominates → policy randomizes
        # BC directly minimizes MSE(policy_mean, expert_actions) with no entropy interference
        bc_loss_value = 0.0
        if bc_optimizer is not None:
            try:
                bc_actor_model = policy.model.action_model if hasattr(policy.model, 'action_model') else policy.model
                _bc_device = next(bc_actor_model.parameters()).device  # device from the model we actually run
                # Handle both numpy arrays and CUDA tensors from learn_on_batch
                def _to_tensor(x, device):
                    if isinstance(x, torch.Tensor):
                        return x.detach().to(device, dtype=torch.float32)
                    return torch.tensor(np.array(x), dtype=torch.float32, device=device)
                obs_tensor = _to_tensor(train_batch['obs'], _bc_device)
                expert_acts = _to_tensor(train_batch['actions'], _bc_device)
                action_dim = expert_acts.shape[-1]
                bc_optimizer.zero_grad()
                dist_out, _ = bc_actor_model({"obs": obs_tensor}, [], None)
                mean_actions = dist_out[:, :action_dim]  # First half of output is mean; second is log_std
                
                bc_loss = torch.mean((mean_actions - expert_acts) ** 2)
                bc_loss.backward()
                bc_optimizer.step()
                ...
                bc_loss_value = 0.0
                if i <= 5:
                    print(f"[WARN] BC step failed at iter {i}: {e}")
        ...
        # Track per-iteration action statistics for KL effectiveness analysis
        try:
            _diag_obs = train_batch['obs'][:200]
            _diag_acts = policy.compute_actions(_diag_obs, explore=False)[0]
            _diag_acts = np.array(_diag_acts)
            action_std_history.append(float(np.std(_diag_acts)))
            action_mean_history.append(float(np.mean(np.abs(_diag_acts))))
        except Exception:
            action_std_history.append(float('nan'))
            action_mean_history.append(float('nan'))
        ...
        if i % EVAL_INTERVAL == 0 or i == 1:
            ...
            print(
                ...
                f"BC Loss: {bc_loss_value:.4f} | KL_β: {current_kl_coeff:.3f} | "
                f"MeanQ: {recent_q:.4f} | "
    ...
    if episode_waypoint_rates:
        ...
        # Create comprehensive burn-in analysis figure with 9 subplots (3x3 grid)
        ...
        # Plot 3: KL Loss (Imitation Loss)
        ax3_kl = fig.add_subplot(gs[0, 2])
        valid_kl_loss = [(i+1, val) for i, val in enumerate(kl_loss_history) if not np.isnan(val)]
        if valid_kl_loss:
            kl_iters, kl_vals = zip(*valid_kl_loss)
            ax3_kl.plot(kl_iters, kl_vals, color='darkviolet', linewidth=1.5)
        ax3_kl.set_title('KL Loss (Policy-Expert MSE)', fontweight='bold', fontsize=12)
        ax3_kl.set_xlabel('Burn-in Iteration')
        ax3_kl.set_ylabel('KL Loss (MSE)')
        ax3_kl.grid(True, alpha=0.3)
        
        # Plot 4: Attention Sharpness
        ...
        # Plot 5: Average MSE (using critic loss as proxy)
        ...
        # Plot 6: KL Coefficient Annealing
        ax4_kl = fig.add_subplot(gs[1, 2])
        ax4_kl.plot(all_iterations, kl_coeff_history, color='brown', linewidth=1.5)
        ax4_kl.set_title('KL Penalty Coefficient (β)', fontweight='bold', fontsize=12)
        ax4_kl.set_xlabel('Burn-in Iteration')
        ax4_kl.set_ylabel('KL Coefficient')
        ax4_kl.grid(True, alpha=0.3)
        ax4_kl.axhline(y=0, color='gray', linestyle='--', alpha=0.5)
        
        # Plot 7: Waypoint Reach Rate
        ...
        # Plot 9: Average Episode Rewards
        if episode_rewards:
        ...
        # ================================================================
        # KL EFFECTIVENESS PLOT
        # Answers: "Did the KL penalty actually teach the actor to imitate?"
        # ================================================================
        fig_kl, axes_kl = plt.subplots(2, 2, figsize=(16, 10))
        fig_kl.suptitle(
            f'KL Imitation Penalty - Effectiveness Analysis\nRun {RUN_ID} | KL_enabled={ENABLE_KL_PENALTY} | β_start={KL_COEFF_START}',
            fontsize=14, fontweight='bold'
        )
        all_iters_kl = list(range(1, len(kl_loss_history) + 1))
        
        # --- Plot A: Action STD over burn-in ---
        # KEY SIGNAL: std should DROP from ~0.19 (random) toward ~0.05-0.10 (imitation)
        # If KL is OFF or ineffective, std stays flat/random
        ax_a = axes_kl[0, 0]
        valid_std = [(j, v) for j, v in zip(all_iters_kl, action_std_history) if not np.isnan(v)]
        if valid_std:
            std_iters, std_vals = zip(*valid_std)
            ax_a.plot(std_iters, std_vals, color='steelblue', linewidth=1.5, label='Action Std')
            # 5-iter moving average
            if len(std_vals) >= 5:
                ma = np.convolve(std_vals, np.ones(5)/5, mode='valid')
                ax_a.plot(std_iters[4:], ma, color='navy', linewidth=2.5, linestyle='--', label='5-iter MA')
        ax_a.axhline(y=0.19, color='red', linestyle=':', alpha=0.7, label='Baseline (random ~0.19)')
        ax_a.axhline(y=0.08, color='green', linestyle=':', alpha=0.7, label='Target (imitating ~0.08)')
        ax_a.set_title('Action Std (↓ = imitating expert)', fontweight='bold')
        ax_a.set_xlabel('Burn-in Iteration')
        ax_a.set_ylabel('Action Std')
        ax_a.legend(fontsize=9)
        ax_a.grid(True, alpha=0.3)
        
        # --- Plot B: KL Loss (MSE policy vs expert) + KL Coefficient ---
        # KEY SIGNAL: KL loss should DROP steadily from high to near-zero
        # The β curve shows when the pressure is applied
        ax_b = axes_kl[0, 1]
        valid_kl = [(j, v) for j, v in zip(all_iters_kl, kl_loss_history) if not np.isnan(v) and v > 0]
        if valid_kl:
            kl_i, kl_v = zip(*valid_kl)
            ax_b.plot(kl_i, kl_v, color='darkviolet', linewidth=1.5, label='KL Loss (MSE)')
        ax_b_twin = ax_b.twinx()
        ax_b_twin.plot(all_iters_kl, kl_coeff_history, color='orange', linewidth=2,
                       linestyle='--', label='KL β coefficient')
        ax_b_twin.set_ylabel('KL Coefficient (β)', color='orange')
        ax_b_twin.tick_params(axis='y', labelcolor='orange')
        ax_b.set_title('KL Loss (↓) + Coefficient Schedule', fontweight='bold')
        ax_b.set_xlabel('Burn-in Iteration')
        ax_b.set_ylabel('KL MSE Loss', color='darkviolet')
        ax_b.tick_params(axis='y', labelcolor='darkviolet')
        lines1, labels1 = ax_b.get_legend_handles_labels()
        lines2, labels2 = ax_b_twin.get_legend_handles_labels()
        ax_b.legend(lines1 + lines2, labels1 + labels2, fontsize=9)
        ax_b.grid(True, alpha=0.3)
        
        # --- Plot C: Actor Loss over burn-in ---
        # KEY SIGNAL: if KL works, actor loss should DECREASE noticeably
        # If it stays flat (~0.02), the actor is not receiving useful gradient
        ax_c = axes_kl[1, 0]
        ax_c.plot(all_iters_kl, actor_loss_history, color='dodgerblue', linewidth=1.2,
                  alpha=0.6, label='Actor Loss')
        if len(actor_loss_history) >= 20:
            ma_actor = np.convolve(actor_loss_history, np.ones(20)/20, mode='valid')
            ax_c.plot(list(range(20, len(actor_loss_history)+1)), ma_actor,
                      color='darkblue', linewidth=2.5, linestyle='--', label='20-iter MA')
        # Shade the region where KL is active
        if ENABLE_KL_PENALTY:
            ax_c.axvspan(1, KL_ANNEAL_UNTIL, alpha=0.08, color='violet', label=f'KL active (1-{KL_ANNEAL_UNTIL})')
        ax_c.set_title('Actor Loss (↓ = KL forcing imitation)', fontweight='bold')
        ax_c.set_xlabel('Burn-in Iteration')
        ax_c.set_ylabel('Actor Loss')
        ax_c.legend(fontsize=9)
        ax_c.grid(True, alpha=0.3)
        
        # --- Plot D: WP Rate vs KL Loss (scatter) ---
        # KEY SIGNAL: as KL loss drops → WP rate should rise
        # A clear negative correlation confirms KL improved the policy
        ax_d = axes_kl[1, 1]
        wp_eval_iters = [EVAL_INTERVAL * (j+1) for j in range(len(episode_waypoint_rates))]
        if valid_kl and episode_waypoint_rates:
            # Interpolate KL loss values at eval iterations
            kl_arr = np.array(kl_loss_history)
            kl_at_eval = []
            for ev_it in wp_eval_iters:
                idx_kl = min(ev_it - 1, len(kl_arr) - 1)
                kl_at_eval.append(kl_arr[idx_kl] if not np.isnan(kl_arr[idx_kl]) else 0.0)
            wp_pct = [wr * 100 for wr in episode_waypoint_rates_plot[:len(kl_at_eval)]]
            sc = ax_d.scatter(kl_at_eval[:len(wp_pct)], wp_pct,
                              c=wp_eval_iters[:len(wp_pct)], cmap='viridis',
                              s=60, zorder=5, label='Eval points')
            plt.colorbar(sc, ax=ax_d, label='Burn-in Iteration')
        ax_d.set_title('KL Loss vs Waypoint Rate\n(lower KL loss → higher WP = KL worked)', fontweight='bold')
        ax_d.set_xlabel('KL Loss (lower = closer to expert)')
        ax_d.set_ylabel('Waypoint Rate (%)')
        ax_d.grid(True, alpha=0.3)
        
        plt.tight_layout()
        kl_plot_path = os.path.join(CHECKPOINT_DIR, f'burn_in_kl_effectiveness_{RUN_ID}.png')
        plt.savefig(kl_plot_path, dpi=150, bbox_inches='tight')
        print(f"   📊 KL effectiveness plot saved to: {kl_plot_path}")
        plt.close()
    ...
    if original_lr is not None and hasattr(policy, '_optimizer'):
        ...
        print(f"   Learning rate restored to original value")
    ...
    return {
        ...
        'kl_coeff_history': kl_coeff_history,
...
def verify_buffer_rewards(algo):
    ...
    # With /12000 scaling (15x stronger than /800):
    # - Perfect expert: +200/12000 = +0.0167
    # - Bad episode: -2000/12000 = -0.167
    # - Typical step: -0.2/12000 = -0.0000167
    # Expected range: [-0.25, +0.02] for current scaling
    
    # Check if rewards are in reasonable range for /12000 scaling
    if max_reward > 0.5:
        print("⚠️ WAARSCHUWING: Max reward > 0.5 suggereert OUDE scaling (bijv. /800 of /500).")
        print("   Buffer is mogelijk gevuld met verkeerde reward schaal.")
    elif max_reward < 0.001:
        print("⚠️ WAARSCHUWING: Max reward < 0.001 suggereert TE KLEINE scaling (bijv. /50000).")
        print("   Gradient signals zijn te zwak voor goede learning.")
    elif min_reward < -1.0:
        print("⚠️ WAARSCHUWING: Min reward < -1.0 suggereert extreme intrusions in buffer.")
        print("   Expert data quality is zeer laag.")
    elif abs(avg_reward) < 0.00001 and std_reward < 0.00001:
        print("⚠️ WAARSCHUWING: Rewards zijn bijna allemaal ~0 - mogelijk lege/corrupte buffer.")
    else:
        print("✅ SUCCESS: Buffer rewards passen bij huidige scaling (/12000).")
        print(f"   Expert data quality: {'uitstekend' if max_reward > 0.01 else 'goed' if avg_reward > -0.005 else 'matig'}")
    print("-" * 30 + "\n")
...
def build_trainer(n_agents):
    ...
    cfg = (
        ...
        .training(
            ...
            target_entropy = -0.5,   # RAISED: -1.0→-0.5 (with Q overestimated to 2.55 and alpha=0.02, entropy term was negligible; need more pressure to prevent pure greedy exploitation)
            ...
            grad_clip=1.0,  # INCREASED: 0.5→1.0 (less aggressive clipping for smaller gradients)
            ...
            # Hyperparameters
            gamma=0.97, # REDUCED: 0.99→0.97 (gamma=0.99 with T=300 steps means Q=r/0.01=100x reward — massive overestimation amplifier; 0.97→Q=r/0.03=33x, 3x less amplification)
            tau=0.003, # REDUCED: 0.005→0.003 (slower target network update reduces Q-overestimation bootstrapping; target tracks slower = less runaway)
            ...
            replay_buffer_config={
                ...
                "prioritized_replay_alpha": 0.6, # exponent determines how much prioritization is used. 
...
if __name__ == "__main__":
    ...
    ray.shutdown()
    time.sleep(2)  # Give OS time to release ports after shutdown (prevents stale-Raylet error on Windows)
    ray.init(
        runtime_env={
    ...
    print(f"   (Ensures reward scale matches current environment: /12000.0)")
    ...
    if ENABLE_BURN_IN:
        if SKIP_BURN_IN_LOAD_CHECKPOINT:
            # ── Restore pre-trained burn-in weights instead of retraining ──
            print(f"\n⚡ SKIP_BURN_IN_LOAD_CHECKPOINT=True — restoring burn-in checkpoint from: {BURN_IN_CHECKPOINT_DIR}")
            try:
                algo.restore(BURN_IN_CHECKPOINT_DIR)
                print(f"✅ Burn-in checkpoint restored successfully.")
            except Exception as e:
                print(f"❌ Failed to restore burn-in checkpoint: {e}")
                print(f"   Falling back to full burn-in training...")
            ...
            # ── Run burn-in normally then save checkpoint for future reuse ──
            burn_in_results = burn_in_on_expert_buffer(
                algo,
                n_iterations=BURN_IN_ITERATIONS,
                batch_size=BURN_IN_BATCH_SIZE,
            ...
            try:
                ...
                pass
            # Save burn-in checkpoint so future runs can skip retraining
            try:
                bi_ckpt = algo.save(BURN_IN_CHECKPOINT_DIR)
                bi_path = bi_ckpt.checkpoint.path if hasattr(bi_ckpt, 'checkpoint') else str(bi_ckpt)
                print(f"💾 Burn-in checkpoint saved to: {bi_path}")
                print(f"   Set SKIP_BURN_IN_LOAD_CHECKPOINT = True to reuse on next run.")
            except Exception as e:
                print(f"⚠️  Could not save burn-in checkpoint: {e}")
