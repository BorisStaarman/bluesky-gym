...
class ForceAlphaCallback(DefaultCallbacks):
    def on_train_result(self, *, algorithm, result, **kwargs):
        # --- CONFIGURATION ---
        START_ALPHA = 0.15 # was 0.2
        MIDDLE_ALPHA = 0.1
        END_ALPHA = 0.06 # was 0.05 in previous good run 
...
class SACExpert:
    def __init__(self, safe_dist_m=125.0, lookahead_s=15.0):
    ...
    for i in range(algo.iteration + 1, target_iters + 1):
        ...
        try:
            ...
            if i % 10 == 0:
                ...
                custom_metrics = result.get("custom_metrics", {})
                wp_rate = custom_metrics.get("waypoint_reached_rate_mean", 0.0) * 100 # Als percentage
                
                # 3. Print de vergelijking
                print("\n" + "="*40)
                print(f"📊 SUCCESS MONITOR - ITERATION {i}")
                print("-"*40)
                ...
                # 4. Interpretatie van de status
                if current_mean_q > 0.70 and wp_rate < 20:
                    print("\n💡 STATUS: Critic is klaar, maar Alpha-ruis blokkeert succes.")
                elif current_mean_q < 0.40:
                    print("\n⚠️ STATUS: Critic verliest vertrouwen in expert-data!")
                else:
                    print("\n✅ STATUS: Model is aan het stabiliseren.")
                print("="*40 + "\n")
