...
def build_trainer(n_agents):
    ...
    cfg = (
        ...
        .training(
            ...
            actor_lr=[
                [0, 5e-5],           # Startwaarde
                [6000, 5e-5],        # Behouden tot de helft
                [12000, 1e-6],        # Verfijnen aan het einde
            ],
            critic_lr=[
                [0, 1e-4],
                [6000, 1e-4],
...
if __name__ == "__main__":
    ...
    for i in range(algo.iteration + 1, target_iters + 1):
        ...
        if i <= 3 or (i == 100):  # Also check at iteration 100
            print(f"\n   [Debug] Iteration {i} custom_metrics keys: {list(custom_metrics.keys())}")
            if custom_metrics:
                print(f"   [Debug] Custom metrics values: {custom_metrics}")
            
            # Also check if metrics are in result at other locations
            if i == 100:
                print(f"   [Debug] Checking other result keys...")
                print(f"   - 'env_runners' keys: {list(result.get('env_runners', {}).keys())[:10]}")
                # Check for any keys containing 'waypoint' or 'intrusion'
                all_keys = str(result.keys())
                if 'waypoint' in str(result).lower() or 'intrusion' in str(result).lower():
                    print(f"   [Debug] Found waypoint/intrusion somewhere in result!")
                    import json
