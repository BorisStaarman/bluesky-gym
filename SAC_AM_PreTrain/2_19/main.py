...
START_ALPHA = 0.05  # REDUCED for /12000 scaling: smaller rewards → entropy more important → need less alpha
...
EXPERT_MIX_DECAY_UNTIL = 0.1  # Decay to 0% by 25% of total iterations (~4000 iters)
...
BURN_IN_BC_ATTENTION_LR = 6e-4       # Attention BC learning rate (W_q, W_k, W_v learn from expert obs)
BURN_IN_BC_TEMPERATURE_LR = 6e-4     # Temperature BC learning rate
...
MAIN_ATTENTION_LR_SCHEDULE = [
    [0, 5e-5],        # REDUCED: 5e-4→5e-5 (high LR immediately destroyed burn-in calibration, sharpness 0.19→0.08)
    [2000, 6e-4],     # Ramp up once actor has stabilized with new online data
    [8000, 4e-4],     # Maintain strong sharpening signal
    [12000, 2e-4],    # Final refinement
]
...
MAIN_TEMPERATURE_LR_SCHEDULE = [
    [0, 5e-5],        # REDUCED: 5e-4→5e-5 (protect burn-in temperature calibration from immediate overwrite)
    [2000, 7e-4],     # Ramp up with attention once online data stabilises
    [8000, 5e-4],
    [12000, 2e-4],
]
...
MAIN_CRITIC_LR_SCHEDULE = [
    [0, 5e-4],        # BOOSTED: 3e-4→5e-4 (smaller Q-values easier to fit)
    [2000, 7e-4],     # BOOSTED: 5e-4→7e-4 (faster learning with small rewards)
    [6000, 4e-4],     # BOOSTED: 2e-4→4e-4 (maintain learning momentum)
    [8000, 2e-4],     # BOOSTED: 1e-4→2e-4 (stabilization)
    [12000, 1e-4],    # BOOSTED: 5e-5→1e-4 (final refinement)
]
...
INTRUSION_CURRICULUM_STAGES = [
    ...
    (4000, -130),    # Stage 2 (4K-8K): Light collision awareness  
    (8000, -175),    # Stage 3 (8K-12K): Moderate penalties - KEEP PRACTICING
    (11000, -250),   # Stage 4 (11K-16K): Full penalties AFTER attention sharpens
]
...
def build_trainer(n_agents):
    ...
    cfg = (
        ...
        .training(
            ...
            target_entropy = -1.0,   # RAISED: -2.0→-1.0 (entropy 2.5 was dominating tiny /12000 rewards; less entropy pressure early on)
            ...
            # Hyperparameters
            gamma=0.99, # REDUCED: 0.995→0.99 (0.995 amplified bootstrapping errors causing Q-divergence: MeanQ 0.077→0.600 while rewards collapsed)
            tau=0.005, # soft update parameter for target    networks, smaller makes target network update more slowly
