...
def main(window: int = 5):
    ...
    # Prepare figure - changed to 2x3 grid to accommodate new metric
    ...
    # 3) Aircraft with intrusions rate (%) - NEW METRIC
    if "aircraft_with_intrusions_rate" in df.columns:
        axes[2].plot(df["iteration"], df["aircraft_with_intrusions_rate"] * 100.0, marker="o", alpha=0.4, label="raw")
        axes[2].plot(df["iteration"], roll["aircraft_with_intrusions_rate"] * 100.0, color="orange", label=f"roll-{window}")
        axes[2].set_title("Aircraft with Intrusions (%)")
        axes[2].set_xlabel("iteration")
        axes[2].set_ylabel("%")
        axes[2].grid(True)
        axes[2].legend()
    else:
        axes[2].text(0.5, 0.5, "aircraft_with_intrusions_rate\nnot available", ha="center", va="center")
        axes[2].set_title("Aircraft with Intrusions (%)")

    # 4) Avg reward
    axes[3].plot(df["iteration"], df["avg_reward"], marker="o", alpha=0.4, label="raw")
    axes[3].plot(df["iteration"], roll["avg_reward"], color="slateblue", label=f"roll-{window}")
    axes[3].set_title("Average reward (eval)")
    ...
    # 5) Avg length
    axes[4].plot(df["iteration"], df["avg_length"], marker="o", alpha=0.4, label="raw")
    axes[4].plot(df["iteration"], roll["avg_length"], color="sienna", label=f"roll-{window}")
    axes[4].set_title("Average episode length (steps)")
    ...
    ac_intr_str = ""
    if "aircraft_with_intrusions_rate" in df.columns:
        ac_intr_str = f" | ac_w_intr={last['aircraft_with_intrusions_rate']*100:.1f}%"
    print(
        f"iter={int(last['iteration'])} | avg_intr={last['avg_intrusions']:.2f} | "
        f"wp_rate={last['waypoint_rate']*100:.1f}%{ac_intr_str} | avg_len={last['avg_length']:.1f} | "
