...
START_ALPHA = 0.11  # Initial alpha value for SAC entropy term
...
# --- Burn-in learning rates (separate for actor and critic) ---
# Actor is kept very small for stability; critic can be slightly larger.
BURN_IN_ACTOR_LR = 7.5e-7
...
class ForceAlphaCallback(DefaultCallbacks):
    ...
    def on_train_result(self, *, algorithm, result, **kwargs):
        ...
        if target_penalty < old_penalty:
            # global CURRENT_INTRUSION_PENALTY
...
def burn_in_on_expert_buffer(algo, n_iterations=2000, batch_size=4096):
    ...
    # --- LEARNING RATE AANPASSING VOOR BURN-IN ---
    # Use separate learning rates for actor and critic during burn-in.
    # If only a single optimizer is available, fall back to a safe shared LR.
    original_lr = None
    original_lrs = None
    if hasattr(policy, '_optimizer'):
        # Single optimizer fallback: store original and set to conservative shared LR
        original_lr = policy._optimizer.param_groups[0]['lr']
        shared_lr = min(BURN_IN_ACTOR_LR, BURN_IN_CRITIC_LR)
        for param_group in policy._optimizer.param_groups:
            param_group['lr'] = shared_lr
        print(f"   Learning rate adjusted (single optimizer): {original_lr:.2e} → {shared_lr:.2e} (for stable burn-in)")
    elif hasattr(policy, 'optimizers'):
        # Multiple optimizers: assume ordering (actor optimizer first, critics later)
        original_lrs = []
        for idx, opt in enumerate(policy.optimizers()):
            original_lrs.append(opt.param_groups[0]['lr'])
            target_lr = BURN_IN_ACTOR_LR if idx == 0 else BURN_IN_CRITIC_LR
            for param_group in opt.param_groups:
                param_group['lr'] = target_lr
        print(f"   Learning rates adjusted: actor={BURN_IN_ACTOR_LR:.2e}, critic={BURN_IN_CRITIC_LR:.2e} (for stable burn-in)")
