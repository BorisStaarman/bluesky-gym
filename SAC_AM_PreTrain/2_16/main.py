...
ENABLE_KL_PENALTY = False      # Add KL penalty during burn-in
KL_COEFF_START = 1.0          # Initial KL coefficient (strong constraint)
KL_COEFF_END = 0.0            # Final KL coefficient (no constraint)
KL_ANNEAL_UNTIL = 3000        # Anneal KL penalty from start to 0 by this iteration (extends beyond burn-in for stability)
...
BURN_IN_ACTOR_LR_LOCAL =2.5e-7       # Keep stable for navigation
...
INTRUSION_CURRICULUM_STAGES = [
    ...
    (11000, -350),   # Stage 4 (11K-16K): Full penalties AFTER attention sharpens
]
...
class ForceAlphaCallback(DefaultCallbacks):
    ...
    def on_train_result(self, *, algorithm, result, **kwargs):
        ...
        ALPHA_FINAL  = 0.015  # UPGRADED: 0.01→0.015 (50% more exploration)
...
def verify_buffer_rewards(algo):
    ...
    # With /500 scaling (matching 2_10 successful burn-in):
    # - Perfect expert: +98/500 = +0.196
    # - Bad episode: -2000/500 = -4.0
    # - Typical step: -0.01/500 = -0.00002
    # Expected range: [-4.0, +0.20] for current scaling
    
    # Check if rewards are in reasonable range for /500 scaling
    if max_reward > 1.0:
        ...
        print("✅ SUCCESS: Buffer rewards passen bij huidige scaling (/500).")
        print(f"   Expert data quality: {'uitstekend' if max_reward > 0.12 else 'goed' if avg_reward > -0.01 else 'matig'}")
    print("-" * 30 + "\n")
