...
TOTAL_ITERS = 3500   # 18000 Maximum total iterations
...
START_ALPHA = 0.20  # Initial alpha value for SAC entropy term

# ============================================================================
# PRETRAINING PARAMETERS - OPTIMIZED FOR SAC BURN-IN STABILITY
# ============================================================================
# Target: ~90K expert samples for 1000 burn-in iterations (batch_size=4096)
# Calculation: 150 episodes × 300 steps × 20 agents × 1.33 (oversample) ≈ 120K samples
PRETRAIN_EPISODES = 20  # Increased from 150 for better state-action coverage
BURN_IN_ITERATIONS = 1000  # Reduced from 1500 to match data availability
                            # With 120K samples and batch 4K, we can do ~30 epochs
                            # 1000 iters = ~33 passes through data (good for convergence)

# --- Expert Mixing Parameters (Linear Decay) ---
EXPERT_MIX_START = 0.30        # Start with 30% expert data at iteration 0
EXPERT_MIX_END = 0.02           # End with 0% expert data
EXPERT_MIX_DECAY_UNTIL = 0.15  # Decay to 0% by 75% of total iterations (375/500)

# --- Burn-in Phase Parameters (Offline Learning from Expert Buffer) ---
# Batch size trades off gradient stability vs. computational efficiency
# Larger = more stable gradients, better for critic learning
# Smaller = more updates per epoch, faster feedback
BURN_IN_BATCH_SIZE = 8192     # Increased from 4096 for more stable critic gradients
                               # With unified LR (1e-4), larger batches prevent overfitting
                               # 8K allows ~15 batches per epoch (120K / 8K)
...
# --- BURN-IN LEARNING RATES: MATCHED TO PPO STAGE 1 PRETRAINING ---
# PPO Stage 1 used unified 1e-4 for all components during imitation learning.
# SAC benefits from the same unified approach during burn-in (offline expert learning).
# This creates a stable foundation before transitioning to online RL.
BURN_IN_TEMPERATURE_LR_LOCAL = 1e-4  # Match PPO: unified LR for temperature
BURN_IN_ACTOR_LR_LOCAL = 1e-4        # INCREASED from 7.5e-7 to match PPO unified LR
BURN_IN_ATTENTION_LR_LOCAL = 1e-4    # Match PPO: unified LR for attention
BURN_IN_CRITIC_LR_LOCAL = 1e-4       # Match PPO: unified LR for critic

# RATIONALE: PPO's unified 1e-4 proved stable for imitation learning.
# SAC burn-in is similar: learning from expert buffer without environment interaction.
# After burn-in, we transition to lower LRs for fine-tuning with online experience.

# --- Main Training Phase Learning Rates (Separate Control) ---
# After burn-in (offline learning at 1e-4), we reduce LRs for stable online RL.
# PPO Stage 2 used 5e-5 after warm-up; SAC starts even lower for safety.

# Actor LR Schedule (for base navigation parameters)
MAIN_ACTOR_LR_SCHEDULE = [
    [0, 5e-6],        # Start conservative after burn-in (20x lower than burn-in)
    [500, 1e-5],      # Gradual ramp-up as critic stabilizes
    [1500, 2e-5],     # Peak learning for collision avoidance
...
# Attention mechanism can handle slightly higher LR than actor base
MAIN_ATTENTION_LR_SCHEDULE = [
    [0, 5e-5],        # Start at 50% of burn-in rate (conservative transition)
    [1000, 3e-5],     # Maintain moderate learning during collision avoidance
    [2500, 2e-5],     # Reduce for stability
    [3500, 1e-5],     # Final fine-tuning
]

# Temperature Parameter LR Schedule (for attention sharpness control)
# Temperature learns attention sharpness; can match attention LR
MAIN_TEMPERATURE_LR_SCHEDULE = [
    [0, 5e-5],        # Match attention LR (conservative start)
    [1000, 3e-5],     # Moderate learning during collision avoidance
    [2500, 2e-5],     # Reduce for stability
    [3500, 1e-5],     # Final fine-tuning
]

# Temperature Parameter LR Schedule (voor agressieve scherpte-opbouw)
# OPTIE VAN GEMINI OM M SNELLER OMHOOG TE LATEN GAAN.
# MAIN_TEMPERATURE_LR_SCHEDULE = [
#     [0, 1e-3],        # "TURBO" start: 5x hoger dan je huidige start
#     [1000, 5e-4],     # Afbouwen naar een krachtige basis
#     [2500, 1e-4],     # Stabiel eindigen
# ]

# Critic LR Schedule (for Q-value learning)
# Critic needs higher LR initially to adapt to online experience after burn-in
MAIN_CRITIC_LR_SCHEDULE = [
    [0, 1e-4],        # Start at burn-in rate to continue learning new Q-values
    [1000, 5e-5],     # Reduce as Q-function stabilizes
    [2500, 3e-5],     # Further reduction for fine-tuning
    [3500, 2e-5],     # Minimal noise in Q-values
]
...
CURRENT_INTRUSION_PENALTY = -100  # Start with Stage 1
...
def apply_dynamic_learning_rates(algorithm, iteration):
    ...
    # Get target learning rates from schedules
...
class ForceAlphaCallback(DefaultCallbacks):
    ...
    def on_train_result(self, *, algorithm, result, **kwargs):
        ...
        # ========================================
        # 1. CURRICULUM LEARNING: DISABLED
        # ========================================
        # Intrusion penalty is fixed at -15 (no dynamic adjustment)
        # if current_iter <= INTRUSION_PENALTY_STAGE_1_END:
        #     target_intrusion_penalty = INTRUSION_PENALTY_STAGE_1
        #     stage_name = "Stage 1 (Easy)"
        # elif current_iter <= INTRUSION_PENALTY_STAGE_2_END:
        #     target_intrusion_penalty = INTRUSION_PENALTY_STAGE_2
        #     stage_name = "Stage 2 (Medium)"
        # else:
        #     target_intrusion_penalty = INTRUSION_PENALTY_STAGE_3
        #     stage_name = "Stage 3 (Hard)"
        
        # CURRICULUM DISABLED: Just log metrics, no penalty changes
        # Haal de huidige prestaties op (check both locations like main loop does)
        ...
        # --- CURRICULUM DISABLED: All penalty logic commented out ---
        """
        # --- OLD CURRICULUM LOGIC (DISABLED) ---
        # We willen 10 iteraties op rij boven de target zitten voor een 'Level Up'
        STREAK_TARGET = 10
        STREAK_TARGET2 = 30
        
        # Track streak BEFORE any updates
        old_streak = self.success_streak
        
        if wp_rate >= 0.95:
            self.success_streak += 1
        else:
            # Reset de streak als hij onder de maat presteert (strenge eis)
            self.success_streak = 0
        
        # Debug print every 50 iterations to monitor streak progress
        if current_iter % 50 == 0:
            print(f"\n   [Curriculum Debug] Iter {current_iter}: WP={wp_rate:.1%}, Streak={self.success_streak}/{STREAK_TARGET}, Penalty={old_penalty:.1f}")

        # Check if streak target was JUST reached (transition from STREAK_TARGET-1 to STREAK_TARGET)
        streak_just_reached = (old_streak == STREAK_TARGET - 1) and (self.success_streak == STREAK_TARGET)
        
        # Bepaal het nieuwe doel op basis van de streak
        stage_name = None
        if self.success_streak >= STREAK_TARGET:
            if old_penalty == -100.0:
                target_penalty = -150.0
                stage_name = f"STREAK BEREIKT: Stage Medium (-100) @ {wp_rate:.1%}"
                print(f"\n🎯 [Curriculum] Streak={self.success_streak} >= {STREAK_TARGET}, upgrading -50 → -100")
                self.success_streak = 0  # Reset AFTER setting target
            elif old_penalty == -150.0 and wp_rate >= 0.96:
                target_penalty = -200.0
                stage_name = f"STREAK BEREIKT: Stage Hard (-150) @ {wp_rate:.1%}"
                print(f"\n🎯 [Curriculum] Streak={self.success_streak} >= {STREAK_TARGET} AND WP≥96%, upgrading -100 → -150")
                self.success_streak = 0  # Reset AFTER setting target

        # Notify when streak target is first reached
        if streak_just_reached:
            print(
                f"\n📣 SUCCESS STREAK REACHED: {STREAK_TARGET} consecutive iters (≥95%) | "
                f"WP Rate: {wp_rate:.1%} | Old penalty: {old_penalty:.1f} | "
                f"Target penalty: {target_penalty:.1f}"
            )

        # Update global penalty if it decreased (more negative = stricter)
        # Remember: -100 < -50 is TRUE (more strict penalty)
        if target_penalty < old_penalty:
            CURRENT_INTRUSION_PENALTY = target_penalty
            print(f"\n📈 PENALTY UPGRADED! {stage_name}")
            print(f"   WP Rate: {wp_rate:.1%} | Streak was: {old_streak} | New Penalty: {target_penalty:.1f}")
            
            # Update env_config so new environments get the updated penalty
            algorithm.config["env_config"]["intrusion_penalty"] = target_penalty
            
            # Update all existing running environments with the new penalty
            def update_env(env):
                base_env = env
                while hasattr(base_env, 'env'): 
                    base_env = base_env.env
                base_env.intrusion_penalty = target_penalty
                return f"Updated to {target_penalty}"
            
            # Try both old and new RLlib worker APIs
            propagated = False
            try:
                # Try new API first (RLlib 2.x)
                if hasattr(algorithm, 'env_runner_group'):
                    algorithm.env_runner_group.foreach_env(update_env)
                    num_workers = algorithm.env_runner_group.num_remote_workers()
                    print(f"   ✅ Penalty propagated via env_runner_group to {num_workers + 1} workers")
                    propagated = True
                # Try old API
                elif hasattr(algorithm, 'workers'):
                    algorithm.workers.foreach_env(update_env)
                    num_workers = algorithm.workers.num_remote_workers()
                    print(f"   ✅ Penalty propagated via workers to {num_workers + 1} workers")
                    propagated = True
            except Exception as e:
                pass  # Silent fail, will print message below
            
            if not propagated:
                print(f"   ⚠️  Could not propagate to workers directly.")
                print(f"   ℹ️  Global CURRENT_INTRUSION_PENALTY updated to {target_penalty:.1f}")
                print(f"   ℹ️  Environments will pick up new value on next reset()")
                
        elif target_penalty != old_penalty:
            # This should never happen (penalty can only decrease or stay same)
            print(f"\n⚠️  [Curriculum WARNING] target_penalty ({target_penalty}) != old_penalty ({old_penalty}) but not less!")
            print(f"   This suggests a bug in the curriculum logic.")
        ...
        """
        # End of disabled curriculum logic
        
        # --- 3. LOGGING VOOR MONITORING ---
        # Log current waypoint rate for monitoring
        result["custom_metrics"]["waypoint_rate_current"] = float(wp_rate)

        # ========================================
        # 2. ALPHA (ENTROPY) SCHEDULE
        # ========================================
        # Nieuwe instellingen voor Run 7
        START_ALPHA = 0.2
        END_ALPHA = 0.06         # Lager eindpunt voor maximale precisie
        FREEZE_UNTIL = 1500 
        DECAY_UNTIL = 3500       # Veel sneller zakken (niet pas bij 6000)
...
def prefill_sac_buffer(algo, n_episodes=30):
    ...
    # --- EXPERT DATA OVERSAMPLING FOR BALANCED BEHAVIOR CLONING ---
    # Goal: Ensure critic learns collision avoidance (rare events) as well as navigation
    # 
    # HEADING_THRESHOLD: Actions with |heading_change| > threshold are considered "maneuvers"
    # - At D_HEADING=45°, threshold=0.03 means actions changing heading by >1.35° (45*0.03)
    # - This captures subtle course corrections while filtering drift compensation
    # - Typical expert: ~30% of actions are maneuvers, ~70% are straight flight
    # 
    # OVERSAMPLE_FACTOR: How many extra copies of maneuver transitions to store
    # - Factor=1.5: Maneuvers stored 1.5x, balances to ~50/50 maneuver/straight ratio
    # - Factor=2: Maneuvers stored 2x, creates ~60/40 maneuver-heavy bias
    # - Higher factors risk overfitting to collision avoidance at cost of navigation
    #
    # With 200 episodes × 300 steps × 20 agents = 1.2M base transitions:
    # - ~30% maneuvers (360K) × 1.5 = 540K maneuver samples
    # - ~70% straight (840K) × 1 = 840K straight samples  
    # - Total: ~1.38M samples (but PER removes duplicates, effective ~120K unique)
    HEADING_THRESHOLD = 0.03  # Threshold for "active steering" (normalized action space)
    OVERSAMPLE_FACTOR = 1.5   # REDUCED from 2 to 1.5 for better balance
                              # Avoids over-representation of maneuvers
    maneuvers_count = 0       # Counter for unique steering moments
    # ----------------------------------------------------------------
    ...
    for ep in range(n_episodes):
        ...
        while env.agents:
            ...
            for aid, action in agent_actions.items():
                ...
                if is_stuur_actie:
                    maneuvers_count += 1
                    # For fractional factors (e.g., 1.5), use probabilistic rounding
                    # Factor 1.5 → 50% chance of storing 2x, 50% chance of 1x (average 1.5x)
                    base_count = int(OVERSAMPLE_FACTOR)  # Floor: 1.5 → 1
                    fractional = OVERSAMPLE_FACTOR - base_count  # 0.5
                    import random
                    repeat_count = base_count + (1 if random.random() < fractional else 0)
                else:
                    repeat_count = 1
...
def burn_in_on_expert_buffer(algo, n_iterations=2000, batch_size=4096):
    ...
    critic_loss_history = []
    actor_loss_history = []
    bc_loss_history = []  # Track behavior cloning loss
    ...
    N_EVAL_EPISODES = 5  # Use 5 episodes per evaluation (quick check)
    ...
    for i in range(1, n_iterations + 1):
        ...
        # --- 2. SKIP BC LOSS: Just train critic normally ---
        # BC loss was causing gradient issues with RLlib's SAC model architecture
        # The model appears to detach gradients internally, preventing backprop
        # Instead, rely on standard SAC training with low alpha (deterministic)
        bc_loss_val = 0.0  # Placeholder for logging
        
        # --- 3. CRITIC TRAINING (Standard SAC TD-learning) ---
        # Suppress alpha for critic training (low entropy = deterministic)
        if alpha_param is not None:
            with torch.no_grad():
                alpha_param.fill_(np.log(0.01))
        
        # Train critic on TD-error
        ...
        # Add BC loss to stats for monitoring (using placeholder since BC loss disabled)
        learner_stats['bc_loss'] = bc_loss_val
        ...
        bc_loss_val = to_scalar(learner_stats.get('bc_loss', float('nan')))
        ...
        bc_loss_history.append(bc_loss_val)
        ...
        if i % EVAL_INTERVAL == 0 or i == 1:
            # Quick evaluation to check waypoint rate and intrusions (suppress verbose output)
            ...
            # --- BEST MODEL TRACKING: Sla gewichten op bij nieuw record ---
            if avg_wp_rate > best_wp_rate:
                prev_best = best_wp_rate  # Store old value before update
                best_wp_rate = avg_wp_rate
                best_iteration = i
                best_weights = copy.deepcopy(policy.get_weights())
                prev_best_pct = (prev_best if prev_best > 0 else 0) * 100
                print(f"   ⭐ NEW BEST MODEL saved! WP: {avg_wp_rate*100:.1f}% (prev: {prev_best_pct:.1f}%)")
            ...
            recent_bc_loss = np.nanmean(bc_loss_history[-10:]) if len(bc_loss_history) >= 10 else bc_loss_val
            print(
            ...
            if smoothed_wp >= 0.80 and avg_intrusions <= 70.00:
    ...
    if episode_waypoint_rates:
        ...
        # Create comprehensive burn-in analysis figure with 6 subplots
    ...
    cfg = (
        ...
        .environment(
            ...
            env_config={
                ...
                # Intrusion penalty fixed at -15 (set in environment file)
            },
        ...
        .training(
            ...
            actor_lr=[
                ...
                [2500, 2e-5],     # Begin verfijning nadat de streak waarschijnlijk is geactiveerd
                [3500, 1e-5],     # Fine-tune fase
            ],
            critic_lr=[
                [0, 3e-4],        # Aggressief leren van de nieuwe penalty-structuur
                [2000, 1e-4],     # Pas verlagen als de Actor een stabiele koers heeft gevonden
                [3500, 5e-5],     # Minimale ruis in de Q-waardes aan het eind
            ],
...
if __name__ == "__main__":
    ...
    for i in range(algo.iteration + 1, target_iters + 1):
        ...
        if i % 50 == 0:
            ...
            # Include the current intrusion penalty (injected by callback)
            intrusion_penalty_val = float(custom_metrics.get(
                "intrusion_penalty_current", CURRENT_INTRUSION_PENALTY
            ))
            print(
                ...
                f"Intrusion Penalty: {intrusion_penalty_val:.1f} | "
