...
EVALUATION_INTERVAL = 200  # e.g., set to 1 or 5 to save during training
...
class ForceAlphaCallback(DefaultCallbacks):
    ...
    def on_train_result(self, *, algorithm, result, **kwargs):
        ...
        START_ALPHA = 0.15
        END_ALPHA = 0.04
        # We gebruiken een vaste waarde voor 8000 iteraties
        FREEZE_UNTIL = 500 
        TOTAL_ITERS_VAL = 8000 
...
def burn_in_on_expert_buffer(algo, n_iterations=2000, batch_size=4096):
    ...
    for i in range(1, n_iterations + 1):
        ...
        if i % EVAL_INTERVAL == 0 or i == 1:
            ...
            print(
                ...
                f"WP Rate: {avg_wp_rate*100:.1f}% | "
                f"Avg Intrusions: {avg_intrusions:.2f}"
            )
            
            # --- EARLY STOPPING: Stop when waypoint rate reaches 70% ---
            if avg_wp_rate >= 0.70:
                print(f"\n🎯 Early stopping: Waypoint rate target reached!")
                print(f"   ✅ WP Rate: {avg_wp_rate*100:.1f}% >= 70%")
    ...
    if episode_waypoint_rates:
        print(f"   Final WP Rate: {episode_waypoint_rates[-1]*100:.1f}%")
        print(f"   Final Avg Intrusions: {episode_intrusions[-1]:.2f}")
    ...
    if episode_waypoint_rates:
        ...
        plot_path = os.path.join(CHECKPOINT_DIR, 'burn_in_metrics.png')
...
if __name__ == "__main__":
    ...
    for i in range(algo.iteration + 1, target_iters + 1):
        ...
        # Haal de waarden op (RLlib voegt vaak '_mean' toe)
        wp_rate = custom_metrics.get("waypoint_rate_mean", 0.0) * 100
        avg_intr = custom_metrics.get("intrusions_mean", 0.0)
