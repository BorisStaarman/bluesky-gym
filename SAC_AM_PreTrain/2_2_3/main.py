...
# --- BURN-IN ACTOR LR SCHEDULE (Prevent Overfitting) ---
# Keep LR constant during learning, then reduce to prevent drift
BURN_IN_ACTOR_LR_SCHEDULE = [
    [0, 1e-6],        # Stage 1: Fast learning phase
    [1500, 1e-6],      # Keep constant until peak expected
    # [801, 5e-7],      # Stage 2: 0.5x reduction for stabilization
    # [1200, 5e-7],     # Keep reduced rate
    # [1201, 2e-7],     # Stage 3: 0.2x fine-tuning to lock in
]

# --- Main Training Phase Learning Rates (Separate Control) ---
# Actor LR Schedule (for base navigation parameters)
# EXTENDED for 12K iterations - no freeze, gradual ramp
...
MAIN_ATTENTION_LR_SCHEDULE = [
    ...
    [6000, 1.5e-4],   # Maintain high for mastery
    [10000, 8e-5],    # Final refinement
]
...
class ForceAlphaCallback(DefaultCallbacks):
    ...
    def on_train_result(self, *, algorithm, result, **kwargs):
        ...
        # --- CURRICULUM DISABLED: Using environment's fixed intrusion penalty ---
        # All curriculum learning logic commented out - environment uses its default penalty value
        ...
        DECAY_UNTIL  = 10000  # Extended: reach final precision at 10K (not 3K)
        ...
        ALPHA_FINAL  = 0.01   # Less aggressive final (1% not 0.5%) - keep some exploration
...
def burn_in_on_expert_buffer(algo, n_iterations=2000, batch_size=4096):
    ...
    # Helper function to get actor LR from schedule
    def get_actor_lr_for_burnin(iteration):
        """Get actor LR from burn-in schedule with linear interpolation"""
        if iteration <= BURN_IN_ACTOR_LR_SCHEDULE[0][0]:
            return BURN_IN_ACTOR_LR_SCHEDULE[0][1]
        if iteration >= BURN_IN_ACTOR_LR_SCHEDULE[-1][0]:
            return BURN_IN_ACTOR_LR_SCHEDULE[-1][1]
        
        for j in range(len(BURN_IN_ACTOR_LR_SCHEDULE) - 1):
            iter_start, lr_start = BURN_IN_ACTOR_LR_SCHEDULE[j]
            iter_end, lr_end = BURN_IN_ACTOR_LR_SCHEDULE[j + 1]
            if iter_start <= iteration <= iter_end:
                progress = (iteration - iter_start) / (iter_end - iter_start)
                return lr_start + (lr_end - lr_start) * progress
        return BURN_IN_ACTOR_LR_SCHEDULE[-1][1]
    ...
    cfg = (
        ...
        .environment(
            ...
            env_config={
                ...
                # Intrusion penalty uses environment's default value
            },
