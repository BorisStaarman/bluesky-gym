...
BURN_IN_ITERATIONS = 2000     # 2000 Number of gradient steps on expert data only
...
# --- Curriculum Learning for Intrusion Penalty ---
# The intrusion penalty will transition through 3 stages during training
# Stage 1: Easy (lower penalty to allow exploration)
# Stage 2: Medium (moderate penalty for balanced learning)
# Stage 3: Hard (full penalty for final performance)
INTRUSION_PENALTY_STAGE_1 = -50.0   # Easy: Start with lower penalty
INTRUSION_PENALTY_STAGE_2 = -55.0   # Medium: Increase penalty
INTRUSION_PENALTY_STAGE_3 = -60.0   # Hard: Full penalty (original value)

# Define when each stage ends (iterations)
INTRUSION_PENALTY_STAGE_1_END = 1500  # Stage 1 lasts until iteration 2000
INTRUSION_PENALTY_STAGE_2_END = 2500  # Stage 2 lasts from 2000 to 4000
# Stage 3 starts at iteration 4000 and continues until training ends
...
CURRENT_INTRUSION_PENALTY = INTRUSION_PENALTY_STAGE_1  # Start with Stage 1
...
class ForceAlphaCallback(DefaultCallbacks):
    ...
    def on_train_result(self, *, algorithm, result, **kwargs):
        ...
        # Haal de huidige prestaties op
        wp_rate = result["custom_metrics"].get("waypoint_rate_mean", 0)
        old_penalty = CURRENT_INTRUSION_PENALTY
        target_penalty = old_penalty

        # --- 1. DYNAMISCHE PENALTY MET STREAK ---
        # We willen 5 iteraties op rij boven de target zitten voor een 'Level Up'
        STREAK_TARGET = 15 
        ...
        if self.success_streak >= STREAK_TARGET:
            if old_penalty > -100.0:
                ...
                stage_name = "STREAK BEREIKT: Stage Medium (-100)"
            elif old_penalty > -150.0 and wp_rate >= 0.94: # Extra drempel voor de zwaarste straf
                target_penalty = -150.0
                stage_name = "STREAK BEREIKT: Stage Hard (-150)"

        # Update alleen bij verandering (Ratchet: nooit terug naar beneden)
        if target_penalty < old_penalty:
            CURRENT_INTRUSION_PENALTY = target_penalty
            print(f"\n📈 {stage_name} | WP Rate: {wp_rate:.1%} | Streak: {self.success_streak}")
...
def prefill_sac_buffer(algo, n_episodes=30):
    ...
    HEADING_THRESHOLD = 0.02  # Drempelwaarde voor 'actieve' stuuracties
    OVERSAMPLE_FACTOR = 10    # Hoeveel vaker we stuuracties opslaan
...
def burn_in_on_expert_buffer(algo, n_iterations=2000, batch_size=4096):
    ...
    # Verlaag de learning rate tijdens burn-in voor stabielere updates
    BURN_IN_LR = 7.5e-7 # was 1.5e-6 in previous, Veel lager dan normale training LR (3e-4)
    
    # Sla originele learning rates op en pas aan
    original_lr = None
    if hasattr(policy, '_optimizer'):
        # SAC gebruikt _optimizer in plaats van optimizer
        original_lr = policy._optimizer.param_groups[0]['lr']
        for param_group in policy._optimizer.param_groups:
            param_group['lr'] = BURN_IN_LR
        print(f"   Learning rate adjusted: {original_lr:.2e} → {BURN_IN_LR:.2e} (for stable burn-in)")
    elif hasattr(policy, 'optimizers'):
        # Alternatief: multiple optimizers (actor/critic apart)
        original_lrs = []
        for opt in policy.optimizers():
            original_lrs.append(opt.param_groups[0]['lr'])
            for param_group in opt.param_groups:
                param_group['lr'] = BURN_IN_LR
        print(f"   Learning rates adjusted to {BURN_IN_LR:.2e} (for stable burn-in)")
    ...
    for i in range(1, n_iterations + 1):
        ...
        if i % EVAL_INTERVAL == 0 or i == 1:
            ...
            if smoothed_wp >= 0.95 and avg_intrusions <= 65.00:
    ...
    if episode_waypoint_rates:
        ...
        # Plot 1: Waypoint reach rate during burn-in
        ...
        if len(episode_waypoint_rates) > 0 and episode_waypoint_rates[0] != episode_waypoint_rates[0]:  # Check for NaN
            eval_iterations = eval_iterations[1:]  # Skip first if NaN
        ...
        ax1.plot(eval_iterations, [wr * 100 for wr in episode_waypoint_rates_plot], 
                marker='o', linewidth=2, color='teal')
        ax1.set_title('Waypoint Reach Rate During Burn-in', fontweight='bold')
        ax1.set_xlabel('Burn-in Iteration')
        ax1.set_ylabel('Waypoint Success Rate (%)')
        ax1.grid(True, alpha=0.3)
        
        # Plot 2: Average intrusions during burn-in
        ax2.plot(eval_iterations, episode_intrusions_plot, 
                marker='o', linewidth=2, color='crimson')
        ax2.set_title('Avg Intrusions During Burn-in', fontweight='bold')
        ax2.set_xlabel('Burn-in Iteration')
        ax2.set_ylabel('Average Intrusions per Episode')
        ax2.grid(True, alpha=0.3)
        
        plt.tight_layout()
        
        # Save the plot (ensure directory exists)
        plot_path = os.path.join(CHECKPOINT_DIR, f'burn_in_metrics_{RUN_ID}.png')
        os.makedirs(os.path.dirname(plot_path), exist_ok=True)
        plt.savefig(plot_path, dpi=150, bbox_inches='tight')
        print(f"   📊 Burn-in metrics plot saved to: {plot_path}")
        plt.close()
    ...
    cfg = (
        ...
        .environment(
            ...
            env_config={
                ...
                # Note: intrusion_penalty will be read from global CURRENT_INTRUSION_PENALTY
                # on each environment reset, enabling curriculum learning
            },
