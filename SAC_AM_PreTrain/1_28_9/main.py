...
BURN_IN_ITERATIONS = 1500     # 2000 Number of gradient steps on expert data only
...
# We gebruiken de waarden die stabiliteit boden, maar met een actieve Critic
# Use the global parameters: BURN_IN_ACTOR_LR, BURN_IN_ATTENTION_LR, BURN_IN_TEMPERATURE_LR
BURN_IN_TEMPERATURE_LR_LOCAL = 1e-4  # Higher LR for temperature to quickly find good sharpness
BURN_IN_ACTOR_LR_LOCAL = 7.5e-7     # Jouw 'ancre' voor navigatie-stabiliteit
BURN_IN_ATTENTION_LR_LOCAL = 1e-4   # Krachtig genoeg voor focus
BURN_IN_CRITIC_LR_LOCAL = 1e-5      # VERHOOGD: De Critic moet de expert snappen!
...
MAIN_ACTOR_LR_SCHEDULE = [
    ...
    [1500, 1e-5],     # Peak learning for collision avoidance   # Fine-tuning
]

# Attention/Temperature LR Schedule (for attention mechanism focus)
MAIN_ATTENTION_LR_SCHEDULE = [
    [0, 1e-4],        # Start with moderate learning
    [1000, 5e-5],     # Increase to learn sharper focus during collision avoidance
...
MAIN_TEMPERATURE_LR_SCHEDULE = [
    [0, 2e-4],        # Start higher to quickly find good sharpness
    [1000, 1e-4],     # Moderate learning during collision avoidance
    [2500, 5e-5],     # Reduce for stability
]
...
MAIN_CRITIC_LR_SCHEDULE = [
    [0, 3e-4],        # Aggressive learning of penalty structure
    [1000, 1e-4],     # Stabilization
    [2500, 5e-5],     # Minimal noise in Q-values
]
...
def prefill_sac_buffer(algo, n_episodes=30):
    ...
    HEADING_THRESHOLD = 0.03  # Drempelwaarde voor 'actieve' stuuracties
