...
TOTAL_ITERS = 90  # Total iterations for RL training
...
def build_trainer(n_agents, restore_path=None):
    """
    Builds the PPO algorithm for RL training with attention model.
    """
    ...
    print("[Config] Using standard PPO for RL with Attention Model")
    
    training_config = {
        "lr": 5e-5,  # Learning rate for RL training
        "train_batch_size": 64000,
        "minibatch_size": 2000,
        "num_sgd_iter": 12,
        "clip_param": 0.2,
        "vf_loss_coeff": 1.0,
        "entropy_coeff": 0.01,  # Exploration coefficient
        "grad_clip": 0.5,
        "gamma": 0.99,
        "lambda_": 0.95,
...
if __name__ == "__main__":
    ...
    # ==============================================================================
    # RL TRAINING (PPO with Attention Model)
    # ==============================================================================
    print(f"\n{'='*60}")
    print(f"🚀 STARTING RL TRAINING: PPO OPTIMIZATION (Maximizing Reward)")
    print(f"{'='*60}")
    
    target_iters = int(TOTAL_ITERS)
    
    # Check if we are trying to resume from a checkpoint
    latest_checkpoint = _find_latest_checkpoint(CHECKPOINT_DIR)
    restored_from = latest_checkpoint if latest_checkpoint else None
    
    if restored_from:
        print(f"🔄 Found existing checkpoint: {restored_from}")
        print("⏭️  Resuming training from checkpoint.")
    else:
        print(f"🆕 Starting fresh training (no checkpoint found)")
    
    # Build Trainer
    print(f"Building Trainer with {N_AGENTS} agents...")
    algo = build_trainer(N_AGENTS, restore_path=restored_from)

    # Update starting iteration count if we restored from a checkpoint
    ...
    for i in range(1, target_iters+1):
        ...
        # Progress display
        print(f"RL Training - Iter {i}/{target_iters} | Reward: {mean_rew:.3f} | Loss: {total_loss:.3f} | Entropy: {entropy:.3f} | VF_Var: {vf_explained_var:.3f}")
