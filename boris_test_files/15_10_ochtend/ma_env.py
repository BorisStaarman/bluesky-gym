...
class SectorEnv(MultiAgentEnv):
    ...
    def _generate_ac(self):
        # Add padding to prevent spawning on the edge (this is still good practice)
        padding = 0.1
        ...
        # --- THE FIX: Sort the agent IDs to ensure deterministic assignment ---
        self.agent_waypoints = {}
        wpts_latlon = [fn.nm_to_latlong(CENTER, p) for p in self.wpts_nm]
        
        # By sorting the agent IDs, we guarantee that 'KL001' is always paired
        # with index 0, 'KL002' with index 1, and so on.
        sorted_agent_ids = sorted(list(self._agent_ids))

        for idx, agent_id in enumerate(sorted_agent_ids):
            # Now, the agent, its waypoint, and its initial position are all
            # correctly linked by the same stable index 'idx'.
    ...
    def _check_drift(self, ac_idx):
        # This function also needs the agent_id to perform the correct lookup
        agent_id = bs.traf.idx2id(ac_idx)
        ac_hdg = bs.traf.hdg[ac_idx]
        wpt_lat, wpt_lon = self.agent_waypoints[agent_id]
