...
METRICS_BASE_DIR = "metrics_17_10"
...
class SectorEnv(MultiAgentEnv):
    ...
    def __init__(self, render_mode=None, n_agents=10, run_id="default"):
        ...
        pid = os.getpid()
        self._metrics_root = os.path.join(METRICS_BASE_DIR, f"run_{self.run_id}", f"pid_{pid}")
        ...
        for agent in active_agents:
            try:
                ...
                parts = [
                    ...
                    np.array(x_r) / MAX_SCENARIO_DIM_M,
                    np.array(y_r) / MAX_SCENARIO_DIM_M,
                    np.array(vx_r) / MAX_RELATIVE_VEL_MS, 
                    np.array(vy_r) / MAX_RELATIVE_VEL_MS,
                    np.array(cos_track), 
                    np.array(sin_track),
