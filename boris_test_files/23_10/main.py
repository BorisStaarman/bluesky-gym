...
TOTAL_ITERS = 40           # Used when starting fresh (no checkpoint)
EXTRA_ITERS = 50           # When resuming, run this many more iterations
FORCE_RETRAIN = True       # Set to False to resume from checkpoint
...
METRICS_DIR = "metrics_23_10"

# --- Scheduling Parameters ---
def get_intrusion_penalty(iteration: int, total_iters: int) -> float:
    """Return intrusion penalty based on current iteration (1/3 curriculum)."""
    third = total_iters // 3
    if iteration <= third:
        return -3.0
...
def get_entropy_coeff(iteration: int, total_iters: int) -> float:
    """Return entropy coefficient with decay schedule."""
    third = total_iters // 3
    if iteration <= third:
        return 0.0125
    elif iteration <= 2 * third:
...
def get_learning_rate(iteration: int, total_iters: int) -> float:
    """Return learning rate with linear decay from 3e-4 to 2e-4."""
...
def build_trainer():
    ...
    initial_entropy = get_entropy_coeff(1, TOTAL_ITERS)
    initial_lr = get_learning_rate(1, TOTAL_ITERS)
...
# ---------------- Evaluation Helpers -----------------
def run_fixed_eval(algo: Algorithm, n_episodes: int = 20, render: bool = False, intrusion_penalty: float = -3.0):
...
if __name__ == "__main__":
    ...
    # Track previous scheduled values to detect changes
    prev_intrusion_penalty = get_intrusion_penalty(algo.iteration if algo.iteration > 0 else 1, target_iters)
    prev_entropy = get_entropy_coeff(algo.iteration if algo.iteration > 0 else 1, target_iters)
    prev_lr = get_learning_rate(algo.iteration if algo.iteration > 0 else 1, target_iters)
    
    # --- Main Training Loop ---
    for i in range(algo.iteration + 1, target_iters + 1):
        # Update schedules if they changed
        curr_intrusion_penalty = get_intrusion_penalty(i, target_iters)
        curr_entropy = get_entropy_coeff(i, target_iters)
        curr_lr = get_learning_rate(i, target_iters)
        
        # Update intrusion penalty in all environments if it changed
        if curr_intrusion_penalty != prev_intrusion_penalty:
            print(f"[Schedule] Iter {i}: Updating intrusion_penalty from {prev_intrusion_penalty} to {curr_intrusion_penalty}")
            ...
            try:
                ...
                if hasattr(algo, 'env_runner_group'):
                    algo.env_runner_group.foreach_env(update_env_penalty)
                elif hasattr(algo, 'workers'):
                    algo.workers.foreach_env(update_env_penalty)
            except Exception as e:
                print(f"  Warning: Could not update env penalty: {e}")
            
            prev_intrusion_penalty = curr_intrusion_penalty
        
        # Update entropy coefficient if it changed
        if abs(curr_entropy - prev_entropy) > 1e-9:
            print(f"[Schedule] Iter {i}: Updating entropy_coeff from {prev_entropy:.4f} to {curr_entropy:.4f}")
            try:
                algo.set_config({"entropy_coeff": curr_entropy})
            except Exception as e:
                print(f"  Warning: Could not update entropy_coeff: {e}")
            prev_entropy = curr_entropy
        
        # Update learning rate if it changed significantly
        if abs(curr_lr - prev_lr) > 1e-9:
            print(f"[Schedule] Iter {i}: Updating lr from {prev_lr:.6f} to {curr_lr:.6f}")
            try:
                algo.set_config({"lr": curr_lr})
            except Exception as e:
                print(f"  Warning: Could not update lr: {e}")
            prev_lr = curr_lr
        ...
        if EVALUATION_INTERVAL and i % EVALUATION_INTERVAL == 0:
            ...
            try:
                ...
                _write_eval_row(metrics=eval_metrics, iteration=i, out_dir=os.path.join(script_dir, METRICS_DIR, f"run_{RUN_ID}"))
            except Exception as e:
