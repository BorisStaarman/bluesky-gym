        ...
        for agent in active_agents:
            try:
                ...
                for i in range(self.num_ac):
                    ...
                    # Calculate distance between ownship and this intruder using UNNORMALIZED coordinates
                    # (dxi, dyi) and (dx, dy) are normalized, so we need actual lat/lon for proper distance
                    _, distance_nm = bs.tools.geo.kwikqdrdist(
                        bs.traf.lat[ac_idx], bs.traf.lon[ac_idx],
                        bs.traf.lat[i], bs.traf.lon[i]
                    )
                    distance = float(distance_nm)  # Distance in nautical miles for proper sorting
