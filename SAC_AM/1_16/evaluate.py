...
if __name__ == "__main__":
    ...
    for episode in range(1, NUM_EVAL_EPISODES + 1):
        ...
        # Count how many aircraft had intrusions (from accumulated intrusion stats)
        aircraft_with_intrusions = sum(1 for agent in env._intrusions_acc if env._intrusions_acc[agent] > 0)
    ...
    # Calculate aircraft with intrusion statistics
    total_aircraft_with_intrusions = sum(episode_aircraft_with_intrusions)
    total_aircraft = NUM_EVAL_EPISODES * N_AGENTS
    aircraft_intrusion_percentage = (total_aircraft_with_intrusions / total_aircraft) * 100
    avg_aircraft_with_intrusions = np.mean(episode_aircraft_with_intrusions)
    ...
    print(f"  - Average Aircraft with Intrusions per Episode: {avg_aircraft_with_intrusions:.2f}/{N_AGENTS}")
    print(f"  - Overall Aircraft Intrusion Rate: {aircraft_intrusion_percentage:.1f}% ({total_aircraft_with_intrusions}/{total_aircraft} aircraft)")
