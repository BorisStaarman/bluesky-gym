...
TOTAL_ITERS = 8000   # 18000 Maximum total iterations
...
class ForceAlphaCallback(DefaultCallbacks):
    def on_train_result(self, *, algorithm, result, **kwargs):
        ...
        MIDDLE_ALPHA = 0.08
        END_ALPHA = 0.04 # was 0.05 in previous good run 
        DECAY_ITERS = TOTAL_ITERS * 0.75
        FREEZE_UNTIL = TOTAL_ITERS * 0.70  # Bevries tot 70%
        DECAY_PERIOD = TOTAL_ITERS * 0.20  # Neem 20% van de tijd om te dalen naar END_ALPHA
...
def run_fixed_eval(algo: Algorithm, n_episodes: int = 20, render: bool = False, n_agents: int = N_AGENTS, silent: bool = True):
    ...
    Returns a dict with avg_reward, avg_length, avg_collisions, waypoint_rate,
    ...
    def _run_episodes():
        ...
        rewards, lengths, collisions, waypoints = [], [], [], []
        ...
        for _ in range(n_episodes):
            ...
            collisions.append(env.total_collisions)  # Track collisions instead of intrusions
            waypoints.append(len(env.waypoint_reached_agents))

        env.close()
        return rewards, lengths, collisions, waypoints
    
    # Run with or without output suppression
    if silent:
        with suppress_output():
            rewards, lengths, collisions, waypoints = _run_episodes()
    else:
        rewards, lengths, collisions, waypoints = _run_episodes()
    ...
    return {
        ...
        "per_episode_collisions": collisions,
        ...
        w.writerow(
            {
                ...
                "avg_collisions": round(metrics["avg_collisions"], 2),
    ...
    for i in range(algo.iteration + 1, target_iters + 1):
        ...
        if EVALUATION_INTERVAL and i % EVALUATION_INTERVAL == 0:
            ...
            try:
                ...
                print(
                    "[Eval] ✅ iter=%d | avg_rew=%.3f | avg_len=%.1f | avg_coll=%.2f | wp_rate=%.1f%%"
                    % (
                        i,
                        eval_metrics["avg_reward"],
                        eval_metrics["avg_length"],
                        eval_metrics["avg_collisions"],
