...
- avg_collisions (lower is better)
...
def main(window: int = 5):
    ...
    # 1) Avg collisions
    axes[0].plot(df["iteration"], df["avg_collisions"], marker="o", alpha=0.4, label="raw")
    axes[0].plot(df["iteration"], roll["avg_collisions"], color="crimson", label=f"roll-{window}")
    axes[0].set_title("Avg collisions per episode")
    ...
    print("\nLast eval row:")
    print(
        f"iter={int(last['iteration'])} | avg_coll={last['avg_collisions']:.2f} | "
