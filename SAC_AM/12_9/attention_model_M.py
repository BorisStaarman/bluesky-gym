...
class AttentionSACModel(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        # DEBUG: Print what the model is initialized with
        print(f"\n[AttentionModel __init__] {name}:")
        print(f"  - Observation space shape: {obs_space.shape}")
        print(f"  - Calculated num_intruders: {self.num_intruders}")
        print(f"  - Expected total obs size: {self.ownship_dim + self.expected_intruder_size}")
        print()
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        # 1. Project Q, K, V individually (Eq 1 in the paper)
        query = self.W_q(own_embed).unsqueeze(1)    # (Batch, 1, 128)
        keys  = self.W_k(int_embed).transpose(1, 2) # (Batch, 128, N) - Transposed K
        values = self.W_v(int_embed)                # (Batch, N, 128)

        # 2. Dot Product (Eq 3 in the paper)
        scores = torch.bmm(query, keys)             # (Batch, 1, N)
        scores = scores / (self.attn_dim ** 0.5)

        # Masking padding (If all features of an intruder are 0, it is padding)
        is_padding = intruder_states.abs().sum(dim=2) < 1e-6
        
        # DEBUG: Print masking info once
        if not hasattr(self, '_debug_printed'):
            self._debug_printed = True
            num_padded = is_padding[0].sum().item()
            num_real = (~is_padding[0]).sum().item()
            print(f"\n[AttentionModel DEBUG]:")
            print(f"  - intruder_states shape: {intruder_states.shape}")
            print(f"  - Padded neighbors: {num_padded}")
            print(f"  - Real neighbors: {num_real}")
            print(f"  - is_padding sample: {is_padding[0].cpu().numpy()}")
            print()
