...
if __name__ == "__main__":
        ...
        while env.agents:
            ...
            for agent_id, agent_obs in obs.items():
                ...
                if non_padding_mask.any():
                    ...
                    intruder_sums += valid_intruders.sum(axis=0)
                    intruder_counts += len(valid_intruders)
    ...
    # Calculate means
    ownship_means = ownship_sums / ownship_counts if ownship_counts > 0 else np.zeros(ownship_features)
    intruder_means = intruder_sums / intruder_counts if intruder_counts > 0 else np.zeros(intruder_features)
    ...
    print("\nOWNSHIP FEATURES:")
    print(f"{'Feature':<15} {'Min':>12} {'Max':>12} {'Mean':>12} {'Range':>12}")
    print("-" * 67)
    for i, name in enumerate(ownship_feature_names):
        min_val = ownship_mins[i]
        max_val = ownship_maxs[i]
        mean_val = ownship_means[i]
        range_val = max_val - min_val
        print(f"{name:<15} {min_val:>12.4f} {max_val:>12.4f} {mean_val:>12.4f} {range_val:>12.4f}")
