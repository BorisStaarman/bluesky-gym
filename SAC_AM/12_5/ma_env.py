...
WAYPOINT_REACHED_REWARD = 2.5  # Reward for reaching waypoint
...
class SectorEnv(MultiAgentEnv):
    ...
    def __init__(self, render_mode=None, n_agents=25, run_id="default",
        ...
        for agent in active_agents:
            try:
                ...
                for i in range(self.num_ac):
                    ...
                    # Risk calculation (for visualization/debugging only)
                    tcpa, dcpa, tLOS = SectorEnv._calculate_conflict_metrics(dx, dy, dvx, dvy)
                    risk = SectorEnv._calculate_risk_score(dcpa, tLOS) if not np.isinf(tLOS) else 0.0
                    
                    # Store (dist, dx, dy, dvx, dvy, cos_trk, sin_trk, risk, id)
                    candidates.append((d_now, dx, dy, dvx, dvy, np.cos(trk), np.sin(trk), risk, other_agent_id))
                ...
                # Update risk levels for rendering
                if candidates:
                    risks_with_ids = [(c[7], c[8]) for c in candidates]
                    risks_with_ids.sort(key=lambda x: -x[0])
                    most_risk, most_risky_id = risks_with_ids[0] if risks_with_ids else (0.0, None)
                ...
                for i in range(NUM_AC_STATE):
                    if i < len(top):
                        ...
                        t = top[i]
                        d_now, dx, dy, dvx, dvy, c_trk, s_trk = t[0], t[1], t[2], t[3], t[4], t[5], t[6]
                        
                        # Normalize locally
                        dx_n = dx / MAX_SCENARIO_DIM_M
                        dy_n = dy / MAX_SCENARIO_DIM_M
                        dvx_n = dvx / MAX_RELATIVE_VEL_MS
                        dvy_n = dvy / MAX_RELATIVE_VEL_MS
                        dist_n = (d_now - DISTANCE_CENTER_M) / DISTANCE_SCALE_M
                        
                        # Append 7 features for this agent
                        intruder_features.extend([dx_n, dy_n, dvx_n, dvy_n, c_trk, s_trk, dist_n])
                    else:
                        # Padding (7 zeros per missing agent)
                        intruder_features.extend([0.0] * 7)

                # Concatenate Ownship + Intruders
                ownship_feats = np.array([cos_drift, sin_drift, airspeed_norm], dtype=np.float32)
    ...
    def _get_obs_feature_names(self):
        ...
        # New structure: interleaved agents
        feature_labels = ["x", "y", "vx", "vy", "cos_trk", "sin_trk", "dist"]
