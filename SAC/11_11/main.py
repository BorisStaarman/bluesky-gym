...
TOTAL_ITERS = 6000  # Maximum total iterations
...
ENABLE_EARLY_STOPPING = False    # Set to False to disable early stopping
EARLY_STOP_PATIENCE = 600        # Number of iterations without improvement before stopping
...
# --- Final Model Saving ---
SAVE_FINAL_MODEL = True  # Set to True to save the model from the last iteration as "final_model"
...
def build_trainer(n_agents):
    ...
    cfg = (
        ...
        .training(
            # LRs
            actor_lr=3e-4, # LR for actor, which decides the actions, small means slower learning but better converging
            critic_lr=1e-3,          # evaluates quality of actions. hihger is better of exploration, 
            # ---- Option A: fixed alpha (stable baseline) ----
            target_entropy = -2.0,   # -2.0 for more exploration. larger negative value is more exploration, but also means less exploitation of policy
            alpha_lr = 1e-6,            # was 3e-5.   lr for updating entropy / alpha. lower means slower alpha updates
            ...
            replay_buffer_config={
                ...
                "capacity": 100_000,  # Reduced from 1M to encourage fresher samples
            },
            num_steps_sampled_before_learning_starts=2_000,  # Reduced from 5000
            train_batch_size=1024,  # Reduced from 2048 for more frequent updates
