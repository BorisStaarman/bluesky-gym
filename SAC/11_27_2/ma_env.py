...
STEP_PENALTY = -0.005  # Small penalty applied every step to encourage efficiency
    ...
    def _get_observation(self, active_agents):
        ...
        for agent in active_agents:
            try:
                ...
                # Build candidate list with conflict detection metrics
                # ONLY include aircraft that have actual conflicts (tLOS < inf)
                candidates = []
                for i in range(self.num_ac):
                    ...
                    # FILTER: Only include aircraft with actual conflicts
                    if not np.isinf(tLOS):
                        risk = SectorEnv._calculate_risk_score(dcpa, tLOS)
                        candidates.append((risk, dx, dy, dvx, dvy, np.cos(trk), np.sin(trk), d_now, other_agent_id))

                # Sort by descending risk (higher risk = higher priority)
                # Break ties by current distance (closer first)
                ...
                # Unpack top-N into arrays, padding with zeros if fewer than NUM_AC_STATE aircraft
                n_neighbors = len(top)
                risk       = [t[0] for t in top] + [0.0] * (NUM_AC_STATE - n_neighbors)
                x_r        = [t[1] for t in top] + [0.0] * (NUM_AC_STATE - n_neighbors)
                y_r        = [t[2] for t in top] + [0.0] * (NUM_AC_STATE - n_neighbors)
                vx_r       = [t[3] for t in top] + [0.0] * (NUM_AC_STATE - n_neighbors)
                vy_r       = [t[4] for t in top] + [0.0] * (NUM_AC_STATE - n_neighbors)
                cos_track  = [t[5] for t in top] + [0.0] * (NUM_AC_STATE - n_neighbors)
                sin_track  = [t[6] for t in top] + [0.0] * (NUM_AC_STATE - n_neighbors)
                distances  = [t[7] for t in top] + [0.0] * (NUM_AC_STATE - n_neighbors)
                ...
                parts = [
                    ...
                    # risk is used for sorting only, not included in observation
                    x_rn,
                    y_rn,
                    vx_rn, 
                    vy_rn,
