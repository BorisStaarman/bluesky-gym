...
def analyze_intrusions(metrics_dir="metrics", run_id=None):
    ...
    # Find the correct intrusions column
    intrusions_col = None
    for col in combined_df.columns:
        if 'intrusion' in col:
            intrusions_col = col
            break
    if not intrusions_col:
    ...
    episode_stats[(intrusions_col, 'mean_roll10')] = (
        episode_stats[(intrusions_col, 'mean')]
        .rolling(window=50, min_periods=1)
        .mean()
        .round(2)
    )
    episode_stats[(intrusions_col, 'sum_roll10')] = (
    ...
    print(f"Average intrusions per episode: {episode_stats[(intrusions_col, 'mean')].mean():.2f}")
    print(f"First 5 episodes avg: {episode_stats[(intrusions_col, 'mean')].head().mean():.2f}")
    print(f"Last 5 episodes avg: {episode_stats[(intrusions_col, 'mean')].tail().mean():.2f}")
    print(f"10-ep rolling mean (first 5): {episode_stats[(intrusions_col, 'mean_roll10')].head().tolist()}")
    print(f"10-ep rolling mean (last 5): {episode_stats[(intrusions_col, 'mean_roll10')].tail().tolist()}")
    improvement = episode_stats[(intrusions_col, 'mean')].head().mean() - episode_stats[(intrusions_col, 'mean')].tail().mean()
