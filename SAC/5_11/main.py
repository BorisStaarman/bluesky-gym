...
EARLY_STOP_PATIENCE = 100        # Number of iterations without improvement before stopping
...
def build_trainer(n_agents):
    ...
    cfg = (
        ...
        .training(
            # LRs
            actor_lr=3e-4,
            critic_lr=1e-3,          # try higher critic LR first
            # Temperature: pick ONE of the two options below

            # ---- Option A: fixed alpha (stable baseline) ----
            target_entropy = -1.5,      # for 2-D action; -2.0 is the default-ish, -1.5 = more entropy
            alpha_lr = 3e-5,            # slow the decay; it fell from 0.20→0.168 too fast
            initial_alpha = 0.5,

            # Do NOT set target_entropy here to keep alpha fixed.

            # ---- Option B (later): auto alpha, slower & proper target) ----
            # target_entropy=-2.0,   # -|A| for 2-D action
            # alpha_lr=1e-5,
