...
class AttentionSACModel(TorchModelV2, nn.Module):
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        if not self.is_critic and hasattr(self, 'log_std'):
            # Clamp log_std to [-3.0, 2.0] to prevent entropy collapse.
            # Without this floor, the PPO policy gradient drives log_std toward -inf
            # over num_sgd_iter steps (exploitation wins over the weak entropy_coeff),
            # causing a limit cycle: entropy collapses -> bad advantage estimates ->
            # gradient noise -> entropy jumps back -> repeat every iteration.
            # Floor of -3.0 allows the warmup's fill_(-2.5) to work as intended
            # (std≈0.082), while still preventing collapse past that point.
            log_std_clamped = torch.clamp(self.log_std, min=-3.0, max=2.0)
            batch_size = out.shape[0]
            log_std_expanded = log_std_clamped.unsqueeze(0).expand(batch_size, -1)
