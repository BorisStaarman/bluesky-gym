...
class MVPDataBridgeCallback(DefaultCallbacks):
    ...
    Extracts:
    - teacher_action: For Stage 1 imitation learning
    - clean_obs: For asymmetric actor-critic (critic uses clean observations)
    """
    def on_postprocess_trajectory(
        ...
        if SampleBatch.INFOS in postprocessed_batch:
            ...
            for info in original_infos:
                ...
                # Extract clean observation for asymmetric actor-critic
                if "clean_obs" in info:
                    ...
                    # No clean obs available - will use noisy obs as fallback
            ...
            # ALWAYS store clean observations for asymmetric actor-critic
            # This ensures batch consistency (all batches must have same keys)
            clean_obs_array = []
            for i, clean_obs in enumerate(clean_observations):
                if clean_obs is not None:
                    clean_obs_array.append(clean_obs)
                else:
                    # Fallback: use the noisy observation from the batch
                    clean_obs_array.append(postprocessed_batch[SampleBatch.OBS][i])
            
            # Always add clean_obs to batch (critical for batch concatenation)
            if len(clean_obs_array) > 0:
                clean_obs_array = np.array(clean_obs_array, dtype=np.float32)
                postprocessed_batch["clean_obs"] = clean_obs_array
            else:
                # Edge case: no clean observations at all, use noisy obs as fallback
                postprocessed_batch["clean_obs"] = postprocessed_batch[SampleBatch.OBS].copy()
...
def stage1_imitation_loss(policy, model, dist_class, train_batch):
    ...
    Supports asymmetric actor-critic by passing clean observations to the model.
    """
    # 1. Prepare input dict for model with clean observations (if available)
    input_dict = {
        SampleBatch.OBS: train_batch[SampleBatch.OBS]
    }
    
    # Inject clean observations for asymmetric actor-critic (critic uses clean obs)
    if "clean_obs" in train_batch:
        input_dict["clean_obs"] = train_batch["clean_obs"]
    
    # 2. Differentiable forward pass with clean obs support
    # logits is a tensor with requires_grad=True
    logits, _ = model(input_dict, [], None)
    
    # 3. Extract only the predicted means (the actions)
    ...
    # 4. Get teacher targets (ensure they are on the correct device)
    ...
    # 5. Calculate raw squared error (element-wise)
...
def build_trainer(n_agents, stage=1, restore_path=None):
    ...
    if stage == 1:
        ...
        # Keep callback active for clean_obs extraction (asymmetric actor-critic)
        current_callbacks = MVPDataBridgeCallback
    ...
    cfg = (
        ...
        .environment(
            ...
            env_config={
                ...
                "kalman_burn_in_steps": 3,  # Use noisy obs for first 5 steps while filter converges
            },
    ...
    # 5. Inject Custom Loss (Both Stages need clean_obs support)
    if stage == 1:
        ...
        # Stage 2: Wrap standard PPO loss to inject clean_obs
        policy = algo.get_policy("shared_policy")
        original_loss_fn = policy.loss
        
        def stage2_loss_wrapper(model, dist_class, train_batch):
            # Inject clean_obs into input_dict for asymmetric actor-critic
            # This ensures the value function gets clean observations
            if "clean_obs" in train_batch:
                # Store original forward method
                original_forward = model.forward
                
                def forward_with_clean_obs(input_dict, state, seq_lens):
                    # Inject clean_obs from train_batch into input_dict
                    if "clean_obs" in train_batch:
                        input_dict["clean_obs"] = train_batch["clean_obs"]
                    return original_forward(input_dict, state, seq_lens)
                
                # Temporarily replace forward method
                model.forward = forward_with_clean_obs
                try:
                    # Call original PPO loss with modified forward
                    loss = original_loss_fn(model, dist_class, train_batch)
                finally:
                    # Restore original forward method
                    model.forward = original_forward
                return loss
            else:
                # No clean_obs available, use standard loss
                return original_loss_fn(model, dist_class, train_batch)
        
        policy.loss = stage2_loss_wrapper
        print("[Config] Stage 2 loss wrapper injected to support asymmetric actor-critic")
...
if __name__ == "__main__":
    ...
    run_stage1 = False if RUN_STAGE_2 else True
