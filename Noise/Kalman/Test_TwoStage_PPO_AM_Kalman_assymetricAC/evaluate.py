...
# Set to False to use raw noisy observations (baseline)
USE_KALMAN_FILTER = True  # ⚠️ Change this to True/False to compare results
...
if USE_BEST_STAGE1_WEIGHTS:
    ...
    CHECKPOINT_DIR = os.path.join(script_dir, "models/sectorcr_ma_sac/best_iter_00028")
...
if __name__ == "__main__":
    ...
    env = SectorEnv(
        ...
        run_id=RUN_ID,
        metrics_base_dir=METRICS_DIR,
        use_kalman_filter=USE_KALMAN_FILTER  # Toggle Kalman filter here
    )
    
    # Print Kalman filter status
    print("\n" + "="*70)
    if USE_KALMAN_FILTER:
        print("🔧 KALMAN FILTER: ENABLED - Using filtered observations")
    else:
        print("⚠️  KALMAN FILTER: DISABLED - Using raw noisy observations")
    print("="*70 + "\n")
