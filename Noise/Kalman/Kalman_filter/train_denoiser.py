...
# 1.  HYPER-PARAMETERS
# ──────────────────────────────────────────────────────────────────────
SEQ_LEN = 3            # sliding window length (SHORTER - drones maneuver rapidly!)
INPUT_DIM = 4          # [x, y, vx, vy]
HIDDEN_DIM = 64        # LSTM hidden units (SMALLER - simpler model)
NUM_LAYERS = 1         # stacked LSTM layers (JUST 1 LAYER)
MLP_HIDDEN = 32        # MLP head hidden width
...
def make_dataset_from_trajectories(trajectories: list, seq_len: int, seed: int = 42):
    ...
    for traj_data in trajectories:
        ...
        # KEY FIX: Window should include CURRENT timestep, predict CURRENT clean state
        # This matches deployment where we have noisy obs up to time t and want clean state at t
        for t in range(seq_len - 1, traj_len - 1):
            # Window: noisy observations [t-seq_len+1, ..., t-1, t]
            # Target: clean state at time t (current)
            # This lets LSTM learn: "given noisy history + noisy current, predict clean current"
            window = noisy[t - seq_len + 1 : t + 1]   # (seq_len, 4) — includes current noisy t
            target = clean[t]                          # (4,) — clean state at current timestep t
            X_all.append(window)
            Y_all.append(target)
...
def train(args):
    ...
    # Physics-based loss weight: encourage velocity-position consistency
    DYNAMICS_WEIGHT = 0.1  # Weight for dynamics consistency loss
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb in train_dl:
            ...
            # Primary loss: MSE between predicted and clean state
            mse_loss = criterion(pred, yb)
            
            # Dynamics consistency loss: position change should match velocity
            # CRITICAL: Must convert units properly!
            # Velocities are normalized by V_NORM (36 m/s)
            # Positions X by X_NORM (8500m), Y by Y_NORM (8000m)
            # prev_state is in normalized space
            prev_state = xb[:, -2, :]  # (batch, 4) at t-1 (noisy but best we have)
            
            # Convert velocity to position change in SAME normalized space
            dt = 1.0  # timestep is 1 second
            # vx_phys = vx_norm * 36 m/s
            # dx_phys = vx_phys * dt = vx_norm * 36 meters
            # dx_norm = dx_phys / 8500 = vx_norm * (36/8500)
            expected_dx = prev_state[:, 2] * (V_NORM / X_NORM) * dt  # vx -> x units
            expected_dy = prev_state[:, 3] * (V_NORM / Y_NORM) * dt  # vy -> y units
            
            # Actual predicted position change
            actual_dx = pred[:, 0] - prev_state[:, 0]  # x[t] - x[t-1]
            actual_dy = pred[:, 1] - prev_state[:, 1]  # y[t] - y[t-1]
            
            # Dynamics loss
            dynamics_loss = torch.mean((actual_dx - expected_dx)**2 + (actual_dy - expected_dy)**2)
            
            # Combined loss
            loss = mse_loss + DYNAMICS_WEIGHT * dynamics_loss
            
            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            epoch_loss += mse_loss.item() * xb.size(0)  # Track MSE for comparability
