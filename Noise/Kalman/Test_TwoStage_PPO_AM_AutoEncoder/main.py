...
# --- Path to pretrained Autoencoder (Phase 2) ---
# Set to None to disable AE noise signal (observation slot will be 0.0).
# After running pretrain_autoencoder.py, point this to the saved model.
...
def _find_latest_checkpoint(base_dir: str) -> str | None:
    """Return the directory path containing rllib_checkpoint.json with latest mtime,
    excluding the stage1_best_weights subfolder (which is Stage 1, not Stage 2).

    Scans base_dir recursively for files named 'rllib_checkpoint.json'. If found,
    returns the parent directory of the newest one; else returns None.
    """
    stage1_dir = os.path.join(base_dir, "stage1_best_weights")
    latest_path = None
    latest_mtime = -1.0
...
def _partial_restore(algo, checkpoint_path: str) -> None:
    """Load policy weights from a checkpoint, padding any mismatched (smaller) tensors
    with zeros so that architecture changes (e.g. ownship_dim 7→8) don't crash.

    This is used when Stage 1 was trained on obs_dim=N but Stage 2 uses obs_dim=N+1.
    The new input column starts as zeros, which is a safe initialisation —
    the policy will learn to use it during RL fine-tuning.
    """
    import pickle
    policy_pkl = os.path.join(checkpoint_path, "policies", "shared_policy", "policy_state.pkl")
    if not os.path.isfile(policy_pkl):
        raise FileNotFoundError(f"policy_state.pkl not found at {policy_pkl}")

    with open(policy_pkl, "rb") as f:
        policy_state = pickle.load(f)

    old_weights = policy_state["weights"]
    policy = algo.get_policy("shared_policy")
    model  = policy.model
    new_state = model.state_dict()

    patched, skipped = [], []
    for key, new_t in new_state.items():
        if key not in old_weights:
            skipped.append(key)
            continue
        old_np = old_weights[key]
        old_t  = torch.tensor(old_np, dtype=new_t.dtype) if not isinstance(old_np, torch.Tensor) else old_np.to(new_t.dtype)
        if old_t.shape == new_t.shape:
            new_state[key] = old_t
        else:
            # Pad with zeros; copy old values into the lower-dimensional slice
            padded = torch.zeros_like(new_t)
            slices = tuple(slice(0, s) for s in old_t.shape)
            padded[slices] = old_t
            new_state[key] = padded
            patched.append(f"{key}: {list(old_t.shape)} → {list(new_t.shape)}")

    model.load_state_dict(new_state, strict=True)

    if patched:
        print(f"[PartialRestore] Padded {len(patched)} weight tensor(s) with zeros:")
        for p in patched:
            print(f"  {p}")
    if skipped:
        print(f"[PartialRestore] {len(skipped)} new key(s) kept at random init: {skipped[:5]}{'...' if len(skipped) > 5 else ''}")

    # Sync to all remote workers via set_weights on the policy
    # (RLlib will also broadcast automatically at the start of the first train() call)
    weights = policy.get_weights()
    policy.set_weights(weights)
    print(f"[PartialRestore] Local policy weights updated; workers will sync on first train() call.")
...
def build_trainer(n_agents, stage=1, restore_path=None):
    ...
    if restore_path:
        ...
        try:
            ...
            if "size mismatch" in str(e) or "does not match" in str(e):
                print(f"[Restore] Shape mismatch detected — using partial restore with zero-padding.")
                print(f"[Restore] Original error: {e}")
                _partial_restore(algo, restore_path)
                # Reset Adam optimizer state: the stale momentum buffers from Stage 1
                # have the old tensor shapes and will crash on the first gradient step.
                policy = algo.get_policy("shared_policy")
                opts = getattr(policy, "_optimizers", None) or []
                if not opts and getattr(policy, "_optimizer", None):
                    opts = [policy._optimizer]
                for opt in opts:
                    opt.state.clear()
                print(f"[Restore] Optimizer state cleared ({len(opts)} optimizer(s)). Adam will restart from zero momentum.")
            else:
                raise
...
if __name__ == "__main__":
    ...
    if latest_checkpoint:
        ...
        if os.path.isfile(os.path.join(s1_ckpt, "rllib_checkpoint.json")):
            ...
            print(f"⚠️  No Stage 2 checkpoint and no stage1_best_weights found. Stage 2 will start from random init.")
            print(f"   (Set RUN_STAGE_2=False to run Stage 1 first, or point restored_from manually.)")
            restored_from = None
