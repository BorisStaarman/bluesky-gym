...
# Check if merged CSV exists, if not, create it
if not os.path.exists(csv_path):
    print(f"Merged CSV not found at: {csv_path}")
    print(f"Creating merged CSV from individual agent files...")
    
    dfs = []
    for path in glob.glob(os.path.join(metrics_dir, "pid_*", "*.csv")):
    ...
    if not dfs:
        print(f"ERROR: No CSV files found in {metrics_dir}/pid_*/*.csv")
        print(f"Make sure training has run and generated episode metrics.")
        exit(1)
    
    df = pd.concat(dfs, ignore_index=True)
    df.sort_values(["finished_at", "agent", "episode_index"], inplace=True)
...
reward_components = [
    ...
    ('sum_reward_proximity', 'Proximity Penalty (total per episode)', 'darkviolet'),
...
cutoff_index = int(len(episode_df) * 0.99)
