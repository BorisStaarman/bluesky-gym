...
if __name__ == "__main__":
    ...
    if run_stage1:
        ...
        for i in range(1, iterations_stage1 + 1): # Run 50 iterations of cloning
            ...
            # --- EVALUATION INTERVAL (Stage 1) ---
            if EVALUATION_INTERVAL and i % EVALUATION_INTERVAL == 0:
                print(f"\n🔄 STAGE 1 EVALUATION at iteration {i}")
                # Run evaluation with teacher comparison
                try:
                    eval_metrics = run_fixed_eval(trainer_stage1, n_episodes=30, n_agents=N_AGENTS, compare_teacher=True)
                    print(f"   [Stage 1 Eval] Avg Reward: {eval_metrics['avg_reward']:.3f} | "
                          f"Avg Intrusions: {eval_metrics['avg_intrusions']:.2f} | "
                          f"Waypoint Rate: {eval_metrics['waypoint_rate']*100:.1f}%")
                    # Write evaluation metrics to CSV (Stage 1 specific directory)
                    eval_out_dir = os.path.join(METRICS_DIR, f"run_{RUN_ID}_stage1")
                    _write_eval_row(eval_metrics, i, eval_out_dir)
                    print(f"   ✅ Stage 1 eval metrics saved")
                except Exception as e:
                    print(f"   [Stage 1 Eval] Error: {e}")
                    import traceback
