...
"""
================================================================================
MULTI-HEAD ADDITIVE ATTENTION ARCHITECTURE (Groot et al. 2025)
================================================================================

This implements the attention mechanism as described in the paper for
multi-agent collision avoidance with waypoint navigation.

ARCHITECTURE:
-------------
1. **Input Splitting**: 
   - Ownship features (7): [cos_drift, sin_drift, speed, x, y, vx, vy]
   - Intruder features (N×5): [dist, dx, dy, dvx, dvy] per intruder

2. **Multi-Head Additive Attention** (3 heads):
   - Query (Q): Projected from ownship state (7 -> 5 per head)
   - Keys (K): Projected from individual intruder features (5 -> 5 per head)
   - Values (V): Projected from individual intruder features (5 -> 5 per head)
   - Energy: tanh(Q + K)  [Additive attention mechanism]
   - Scores: v^T * Energy  [Project to scalar]
   - Weights: softmax(scores) with padding mask
   - Context: Weighted sum of values
   - Output: 15-dim context vector (3 heads × 5 features)

3. **Feature Concatenation**:
   - Ownship features (7) concatenated with attention context (15)
   - Input to policy/value networks: 22 features total
...
KEY INSIGHT:
------------
The ownship state (including waypoint drift) is used as the QUERY in attention.
This allows the network to learn: "Given MY current state and goal direction,
which intruders should I pay attention to for collision avoidance?"

NOTE: This matches the paper implementation exactly.
================================================================================
"""

class AttentionSACModel(TorchModelV2, nn.Module):
    """
    Multi-Head Additive Attention (Groot et al. 2025).
    - Uses 3 independent heads with additive attention mechanism
    - Query: From ownship state (includes waypoint drift)
    - Keys/Values: From intruder states
    - Output: Ownship features concatenated with attention context
    """
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        self.intruder_dim = 5 # [dist, dx, dy, dvx, dvy] (MUST MATCH _get_observation)
        ...
        # 2. Multi-Head Additive Attention Layers (Per Paper)
        # Query: From ownship state (ego features)
        # Keys/Values: From intruder states
        
        # W_q: Projects Ownship (7) -> Head Dim (5) [AS PER PAPER]
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        for h in range(self.num_heads):
            ...
            # --- A. Linear Projections (AS PER PAPER) ---
            # Q: Project ownship state (ego features including waypoint drift)
            # This allows attention to ask: "Given MY state, which intruders are relevant?"
            # Shape: (Batch, 7) -> project -> (Batch, 5) -> unsqueeze -> (Batch, 1, 5)
            query_h = self.W_q_heads[h](ownship_state).unsqueeze(1)  # (Batch, 1, 5)
        ...
        # 4. Feature Concatenation: Combine Ownship with Attention Context
        # Ownship features are concatenated with the attention output
