...
class SectorEnv(MultiAgentEnv):
    ...
    def __init__(self, render_mode=None, n_agents=30, run_id="default",
        ...
        # Store clean (ground truth) observations for asymmetric actor-critic
        # Format: {agent_idx: {'pos': [x_clean, y_clean], 'vel': [vx_clean, vy_clean], 'gs_clean': float}}
        self._clean_states = {}
        
        # Kalman filter instances per agent (initialized in reset)
        self._kalman_filters = {}
        
        # Position error tracking
        self._position_errors = defaultdict(list)  # {agent_id: [error1, error2, ...]}
        self._velocity_errors = defaultdict(list)  # {agent_id: [error1, error2, ...]}
        self._error_metrics_buffer = []  # Buffer for CSV writing
        self._error_timesteps = defaultdict(list)  # {agent_id: [timestep1, timestep2, ...]}
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Initialize Kalman filters for each agent (if enabled)
        if self.use_kalman_filter:
            self._kalman_filters = {}
            for i in range(self.num_ac):
                self._kalman_filters[i] = KalmanDenoiser(process_noise_std=1.0)
        
        # Reset error tracking for new episode
        self._position_errors = defaultdict(list)
        self._velocity_errors = defaultdict(list)
        ...
        # Generate clean states first (ground truth)
        self._generate_clean_states()
        
        # Generate noisy observations from clean states (ONCE per timestep)
        self._generate_noisy_observations()
        
        # Actor gets noisy/filtered observations (default behavior)
        ...
        # Generate clean observations for critic
        ...
        for agent in self.agents:
            ...
            # Pass clean observation to critic for asymmetric actor-critic
    ...
    def step(self, actions):
        ...
        for agent_id in agents_in_step:
            ...
            # 4. Pass clean observation to critic for asymmetric actor-critic
        ...
        if len(self.agents) == 0:
            ...
            # DISABLED: write position error metrics CSV at episode end
            # try:
            #     self._write_position_error_metrics_csv()
            # except Exception as e:
            #     print(f"[position-error-metrics] failed to write CSV at episode end: {e}")
            # DISABLED: Generate position error plot at episode end
            # try:
    ...
    def _write_position_error_metrics_csv(self):
        """Write position and velocity error statistics to CSV file."""
        if not self._position_errors:
            return
        
        # Calculate statistics for all agents
        summary_rows = []
        for agent_id in self._position_errors.keys():
            pos_errors = self._position_errors.get(agent_id, [])
            vel_errors = self._velocity_errors.get(agent_id, [])
            ...
            summary_rows.append({
                ...
                "agent_id": agent_id,
                "num_observations": len(pos_errors),
                "pos_error_mean": float(np.mean(pos_errors_arr)),
                "pos_error_std": float(np.std(pos_errors_arr)),
                "pos_error_min": float(np.min(pos_errors_arr)),
                "pos_error_max": float(np.max(pos_errors_arr)),
                "pos_error_median": float(np.median(pos_errors_arr)),
                "vel_error_mean": float(np.mean(vel_errors_arr)),
                "vel_error_std": float(np.std(vel_errors_arr)),
                "vel_error_min": float(np.min(vel_errors_arr)),
                "vel_error_max": float(np.max(vel_errors_arr)),
                "vel_error_median": float(np.median(vel_errors_arr)),
                "kalman_enabled": self.use_kalman_filter,
            })
        ...
        # Write to CSV
        path = os.path.join(self._metrics_root, "position_error_metrics.csv")
        write_header = not os.path.exists(path)
        
        if write_header:
            print(f"[position-error-metrics] Creating CSV: {os.path.abspath(path)}")
        
        with open(path, "a", newline="") as f:
            fieldnames = [
                "episode_index", "agent_id", "num_observations",
                "pos_error_mean", "pos_error_std", "pos_error_min", "pos_error_max", "pos_error_median",
                "vel_error_mean", "vel_error_std", "vel_error_min", "vel_error_max", "vel_error_median",
                "kalman_enabled"
            ]
    ...
    def _plot_position_errors(self):
        """Generate and save position error plots as PNG."""
        if not self._position_errors:
            return
        ...
        try:
            ...
            print("[position-error-plot] matplotlib not available, skipping plot generation")
            return
        
        # Create figure with 2 subplots
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10))
        
        # Colors for agents (use a colormap)
        num_agents = len(self._position_errors)
        colors = plt.cm.tab20(np.linspace(0, 1, num_agents))
        
        # Collect all errors and timesteps for statistics
        all_pos_errors = []
        all_vel_errors = []
        
        # Plot 1: Position errors over time for each agent
        for idx, (agent_id, errors) in enumerate(self._position_errors.items()):
            if not errors:
                continue
            timesteps = self._error_timesteps.get(agent_id, list(range(len(errors))))
            ax1.plot(timesteps, errors, alpha=0.6, linewidth=0.8, 
                    color=colors[idx % len(colors)], label=agent_id if idx < 5 else None)
            all_pos_errors.extend(errors)
        
        # Add mean line
        if all_pos_errors:
            mean_pos_error = np.mean(all_pos_errors)
            ax1.axhline(y=mean_pos_error, color='red', linestyle='--', linewidth=2, 
                       label=f'Mean: {mean_pos_error:.3f} m')
            ax1.axhline(y=3.5, color='gray', linestyle=':', linewidth=1.5, 
                       label='Expected noise std (3.5 m)')
        
        ax1.set_xlabel('Timestep', fontsize=12)
        ax1.set_ylabel('Position Error (m)', fontsize=12)
        filter_status = "WITH Kalman Filter" if self.use_kalman_filter else "WITHOUT Kalman Filter (Noisy)"
        ax1.set_title(f'Position Error Over Time - {filter_status}', fontsize=14, fontweight='bold')
        ax1.grid(True, alpha=0.3)
        ax1.legend(loc='upper right', fontsize=8)
        
        # Plot 2: Velocity errors over time for each agent
        for idx, (agent_id, errors) in enumerate(self._velocity_errors.items()):
            if not errors:
                continue
            timesteps = self._error_timesteps.get(agent_id, list(range(len(errors))))
            ax2.plot(timesteps, errors, alpha=0.6, linewidth=0.8, 
                    color=colors[idx % len(colors)], label=agent_id if idx < 5 else None)
            all_vel_errors.extend(errors)
        
        # Add mean line
        if all_vel_errors:
            mean_vel_error = np.mean(all_vel_errors)
            ax2.axhline(y=mean_vel_error, color='red', linestyle='--', linewidth=2, 
                       label=f'Mean: {mean_vel_error:.3f} m/s')
            ax2.axhline(y=0.1, color='gray', linestyle=':', linewidth=1.5, 
                       label='Expected noise std (0.1 m/s)')
        
        ax2.set_xlabel('Timestep', fontsize=12)
        ax2.set_ylabel('Velocity Error (m/s)', fontsize=12)
        ax2.set_title(f'Velocity Error Over Time - {filter_status}', fontsize=14, fontweight='bold')
        ...
        # Save plot
        plot_path = os.path.join(self._metrics_root, f"position_error_plot_episode_{self._episode_index}.png")
        plt.savefig(plot_path, dpi=150, bbox_inches='tight')
        plt.close(fig)
        
        print(f"\n[position-error-plot] Saved plot to: {os.path.abspath(plot_path)}")
        
        # Print summary statistics
        if all_pos_errors:
            print(f"[position-error-plot] Position Error - Mean: {np.mean(all_pos_errors):.3f} m, "
                  f"Std: {np.std(all_pos_errors):.3f} m, "
                  f"Min: {np.min(all_pos_errors):.3f} m, "
                  f"Max: {np.max(all_pos_errors):.3f} m")
        if all_vel_errors:
            print(f"[position-error-plot] Velocity Error - Mean: {np.mean(all_vel_errors):.3f} m/s, "
                  f"Std: {np.std(all_vel_errors):.3f} m/s")
    ...
    # flush any leftovers for all agents
        ...
        # DISABLED: write position error metrics CSV once
        # try:
        #     self._write_position_error_metrics_csv()
        # except Exception as e:
        #     print(f"[position-error-metrics] failed to write CSV: {e}")
        # DISABLED: Generate final position error plot
        # try:
        #     self._plot_position_errors()
        # except Exception as e:
    ...
    def _generate_clean_states(self):
        """
        Generate clean (ground truth) observations for ALL agents.
        This stores the true position and velocity without any noise.
        
        Called BEFORE _generate_noisy_observations() to establish ground truth.
        
        Stores results in self._clean_states dictionary:
        {agent_idx: {'pos': [x_clean_m, y_clean_m], 'vel': [vx_clean_ms, vy_clean_ms], 'gs_clean': float}}
        """
        self._clean_states = {}
        ...
        for i in range(self.num_ac):
            ...
            # Get true ground speed and heading
            true_gs = bs.traf.gs[i]  # m/s
            hdg = bs.traf.hdg[i]
            
            # Calculate true velocity components
            true_vx = np.cos(np.deg2rad(hdg)) * true_gs
            true_vy = np.sin(np.deg2rad(hdg)) * true_gs
            
            # Store clean observations for this agent
            self._clean_states[i] = {
                'pos': true_loc.copy(),  # [x, y] in meters
                'vel': np.array([true_vx, true_vy]),  # [vx, vy] in m/s
                'gs_clean': true_gs  # scalar ground speed in m/s
            }
    ...
    def _generate_noisy_observations(self):
        ...
        IMPORTANT: This method assumes _generate_clean_states() has been called first,
        so that self._clean_states contains the ground truth for all agents.
        
        Process:
        1. Takes clean states from self._clean_states
        2. Adds noise to create noisy observations
        3. Applies Kalman filtering if self.use_kalman_filter is True
        4. Stores filtered/noisy results in self._noisy_states
        
        Stores results in self._noisy_states dictionary:
        {agent_idx: {'pos': [x_filtered/noisy_m, y_filtered/noisy_m], 'vel': [vx, vy], 'gs_noisy': float}}
        """
        self._noisy_states = {}
        
        for i in range(self.num_ac):
            # Get clean (ground truth) state - must have been generated first!
            if i not in self._clean_states:
                raise RuntimeError(f"Clean states not generated for agent {i}. Call _generate_clean_states() first!")
            
            clean_state = self._clean_states[i]
            true_loc = clean_state['pos']
            true_gs = clean_state['gs_clean']
            true_vx, true_vy = clean_state['vel']
            
            # Add noise to position (3.5m standard deviation per axis)
            pos_noise = np.random.normal(0, 3.5, size=2)
            noisy_loc = true_loc + pos_noise
            
            # Add noise to velocity components (0.1 m/s standard deviation per component)
            vel_noise = np.random.normal(0, 0.1, size=2)
            vx_noisy = true_vx + vel_noise[0]
            vy_noisy = true_vy + vel_noise[1]
            noisy_gs = np.sqrt(vx_noisy**2 + vy_noisy**2)
            
            # Apply Kalman filtering if enabled
            if self.use_kalman_filter:
                # Get or initialize Kalman filter for this agent
                if i not in self._kalman_filters:
                    self._kalman_filters[i] = KalmanDenoiser(process_noise_std=1.0)
                
                kf = self._kalman_filters[i]
                
                # Normalize noisy measurement for Kalman filter
                noisy_normalized = np.array([
                    noisy_loc[0] / 8500.0,  # x normalized
                    noisy_loc[1] / 8000.0,  # y normalized
                    vx_noisy / 36.0,         # vx normalized
                    vy_noisy / 36.0          # vy normalized
                ], dtype=np.float32)
                
                # Initialize filter on first step
                if not hasattr(kf, 'x') or kf.x is None:
                    # Reset filter with first measurement
                    kf.reset(noisy_normalized)
                    # Immediately run predict-update to get a filtered estimate
                    # (better than returning pure noise on first step)
                    kf.predict()
                    kf.update(noisy_normalized)
                    filtered_normalized = kf.x.copy()
                else:
                    # Predict and update for real-time filtering
                    kf.predict()
                    kf.update(noisy_normalized)
                    filtered_normalized = kf.x.copy()
                
                # Denormalize filtered state
                filtered_loc = np.array([
                    filtered_normalized[0] * 8500.0,  # x in meters
                    filtered_normalized[1] * 8000.0   # y in meters
                ])
                filtered_vx = filtered_normalized[2] * 36.0  # vx in m/s
                filtered_vy = filtered_normalized[3] * 36.0  # vy in m/s
                
                # Calculate position and velocity errors (filtered vs true)
                pos_error_filtered = np.linalg.norm(filtered_loc - true_loc)
                vel_error_filtered = np.linalg.norm(np.array([filtered_vx, filtered_vy]) - np.array([true_vx, true_vy]))
                
                # DISABLED: Store error metrics for this agent
                # agent_id = self.agents[i] if i < len(self.agents) else f"agent_{i}"
                # self._position_errors[agent_id].append(pos_error_filtered)
                ...
                if self._env_step < self.kalman_burn_in_steps:
                    ...
                    # After burn-in: use filtered observations
                ...
                # Calculate position and velocity errors (noisy vs true)
                pos_error_noisy = np.linalg.norm(noisy_loc - true_loc)
                vel_error_noisy = np.linalg.norm(np.array([vx_noisy, vy_noisy]) - np.array([true_vx, true_vy]))
                
                # DISABLED: Store error metrics for this agent
                # agent_id = self.agents[i] if i < len(self.agents) else f"agent_{i}"
                # self._position_errors[agent_id].append(pos_error_noisy)
                # self._velocity_errors[agent_id].append(vel_error_noisy)
                # self._error_timesteps[agent_id].append(self._env_step)
            
            # Store final observations for this agent
            self._noisy_states[i] = {
                'pos': final_loc,  # [x, y] in meters (filtered if Kalman enabled, noisy otherwise)
                'vel': np.array([final_vx, final_vy]),  # [vx, vy] in m/s
                'gs_noisy': final_gs  # scalar ground speed in m/s
            }
        
        # VERIFICATION: Print asymmetric AC noise check - REDUCED FREQUENCY for training
        if not hasattr(self, '_noise_check_counter'):
            self._noise_check_counter = 0
        self._noise_check_counter += 1
        
        # Only print every 500 steps AND only in first 3 episodes (to verify asymmetric AC is working)
        if self._noise_check_counter % 500 == 1 and self._episode_index <= 3 and 0 in self._noisy_states and 0 in self._clean_states:
            agent_idx = 0
            
            # Get clean (ground truth) state
            clean_loc = self._clean_states[agent_idx]['pos']
            clean_vel = self._clean_states[agent_idx]['vel']
            
            # Get noisy/filtered state (what actor sees)
            actor_loc = self._noisy_states[agent_idx]['pos']
            actor_vel = self._noisy_states[agent_idx]['vel']
            
            # Calculate errors
            pos_error = np.linalg.norm(actor_loc - clean_loc)
            vel_error = np.linalg.norm(actor_vel - clean_vel)
            
            filter_status = "WITH Kalman Filter" if self.use_kalman_filter else "WITHOUT Kalman Filter (Noisy)"
            
            print("\n" + "="*80)
            print(f"ASYMMETRIC AC NOISE CHECK - Episode {self._episode_index}, Step {self._env_step}")
            print(f"Filter Status: {filter_status}")
            print(f"Agent 0 (KL001):")
            print(f"  Clean Position (Critic):  [{clean_loc[0]:7.2f}, {clean_loc[1]:7.2f}] m")
            print(f"  Actor Position ({filter_status}): [{actor_loc[0]:7.2f}, {actor_loc[1]:7.2f}] m")
            print(f"  Position Error: {pos_error:.2f} m")
            print(f"  Clean Velocity (Critic):  [{clean_vel[0]:6.3f}, {clean_vel[1]:6.3f}] m/s")
            print(f"  Actor Velocity ({filter_status}): [{actor_vel[0]:6.3f}, {actor_vel[1]:6.3f}] m/s")
            print(f"  Velocity Error: {vel_error:.3f} m/s")
            print("="*80 + "\n")
    
    def _get_observation(self, active_agents, use_clean_obs=False):
        """
        Build observation vectors for agents.
        
        Args:
            active_agents: List of agent IDs to generate observations for
            use_clean_obs: If True, use clean (ground truth) states for observation.
                          If False (default), use noisy/filtered states.
                          
        Returns:
            Dictionary mapping agent_id -> observation vector
            
        Note:
            - Actor calls this with use_clean_obs=False (gets noisy/filtered obs)
            - Critic calls this with use_clean_obs=True (gets clean obs)
            - This implements asymmetric actor-critic training
        """
        ...
        # Select which state dictionary to use
        state_dict = self._clean_states if use_clean_obs else self._noisy_states
        ...
        for agent in active_agents:
            try:
                ...
                # Get ownship position and velocity from selected state dictionary
                # (either clean for critic or noisy/filtered for actor)
                if ac_idx in state_dict:
                    ac_loc = state_dict[ac_idx]['pos']  # Position from selected dict
                    vx, vy = state_dict[ac_idx]['vel']  # Velocity from selected dict
                    ac_velocity = state_dict[ac_idx].get('gs_clean' if use_clean_obs else 'gs_noisy')  # Ground speed
                else:
                    # Fallback if states not generated (shouldn't happen)
                    ac_loc = fn.latlong_to_nm(self.center, np.array([bs.traf.lat[ac_idx], bs.traf.lon[ac_idx]])) * NM2KM * 1000
                ...
                for i in range(self.num_ac):
                    ...
                    # Get intruder position and velocity from selected state dictionary
                    # (either clean for critic or noisy/filtered for actor)
                    if i in state_dict:
                        int_loc = state_dict[i]['pos']  # Position from selected dict
                        vxi, vyi = state_dict[i]['vel']  # Velocity from selected dict
                    else:
                        # Fallback if states not generated (shouldn't happen)
                        int_hdg = bs.traf.hdg[i]
                        int_loc = fn.latlong_to_nm(self.center, np.array([bs.traf.lat[i], bs.traf.lon[i]])) * NM2KM * 1000
