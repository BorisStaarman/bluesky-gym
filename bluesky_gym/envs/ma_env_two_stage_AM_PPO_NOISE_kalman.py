...
class SectorEnv(MultiAgentEnv):
    ...
    def _generate_noisy_observations(self):
        ...
        Optionally applies Kalman filtering if self.use_kalman_filter is True.
        ...
        for i in range(self.num_ac):
            ...
            if self.use_kalman_filter:
                ...
                if not hasattr(kf, 'x') or kf.x is None:
                    ...
                    filtered_normalized = noisy_normalized
                else:
                ...
                # Store error metrics for this agent
        ...
        # VERIFICATION: Print noise/filter check - REDUCED FREQUENCY for training
        ...
        if self._noise_check_counter % 500 == 1 and self._episode_index <= 3 and 0 in self._noisy_states:
            ...
            observed_loc = self._noisy_states[agent_idx]['pos']
            pos_error = np.linalg.norm(observed_loc - true_loc)
            
            true_gs = bs.traf.gs[agent_idx]
            observed_gs = self._noisy_states[agent_idx]['gs_noisy']
            vel_error = abs(observed_gs - true_gs)
            
            filter_status = "FILTERED" if self.use_kalman_filter else "NOISY"
            print(f"\n[{filter_status} CHECK - Step {self._noise_check_counter}] Agent {self.agents[0] if self.agents else 'N/A'}:")
            print(f"  True position:     [{true_loc[0]:7.2f}, {true_loc[1]:7.2f}] m")
            print(f"  Observed position: [{observed_loc[0]:7.2f}, {observed_loc[1]:7.2f}] m")
            print(f"  Position error: {pos_error:.3f} m")
            print(f"  True speed:     {true_gs:.3f} m/s")
            print(f"  Observed speed: {observed_gs:.3f} m/s")
            print(f"  Speed error:    {vel_error:.3f} m/s")
