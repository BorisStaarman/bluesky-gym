...
# 1 km^2 = 0.291553706 NM^2 of 1 NM^2 = 3.42990449 km^2
AC_DENSITY_MU = 4 # In AC/NM^2. 4 aircraft in 1000M^2 = 13.7 according to chat
AC_DENSITY_RANGE = (AC_DENSITY_MU -1, AC_DENSITY_MU + 1) # In AC/NM^2. 
AC_DENSITY_SIGMA = 0.5 # In AC/NM^2

POLY_AREA_RANGE = (2, 3) # In NM^2
...
INTRUSION_DISTANCE = 0.054 # NM. 100 meters = 0.054 NM

# Model parameters
ACTION_FREQUENCY = 5 # Number of simulation steps between each action taken by the agent, action stays the same during these timesteps
NUM_AC_STATE = 4  # Number of intruder aircraft the agent observes 
DRIFT_PENALTY = -0.1 
INTRUSION_PENALTY = -1
# scaling factors for the action space
# originally D_HEADING = 22.5 and D_VELOCITY = 20/3
# since drones can turn faster, we increase D_HEADING to 45 degrees
# since drones fly slower, we decrease D_VELOCITY 
D_HEADING = 45 # deg, scaling factor of heading
D_VELOCITY = 10/3 # kts, max increase or decrease of velocity

class SectorCREnv_boris(gym.Env):
    """ 
    ...
    def _render_frame(self):
        ...
        # code to ensure window keeps responding to events
        if self.render_mode == "human":
            # Verwerk de event queue om "Not Responding" te voorkomen
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    # Zorgt ervoor dat je het venster kunt sluiten zonder crash
                    pygame.quit()
                    # Idealiter zou je hier sys.exit() aanroepen, maar dit is voor nu genoeg
