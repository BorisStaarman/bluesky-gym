...
two_stage_am_dir = r"C:\Users\boris\Documents\bsgym\bluesky-gym\Noise\Kalman\PPO_Symmetric_Kalman"
...
def sector_env_creator(config):
    cfg = dict(config or {})
    try:
        sig = inspect.signature(SectorEnv.__init__)
        has_var_kwargs = any(
            p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()
        )
        if has_var_kwargs:
            return SectorEnv(**cfg)
        ...
        filtered_cfg = {k: v for k, v in cfg.items() if k in accepted}
        dropped = sorted(set(cfg.keys()) - set(filtered_cfg.keys()))
        if dropped:
            print(f"[export] Ignoring unsupported env config keys: {dropped}")
        return SectorEnv(**filtered_cfg)
    except Exception:
        return SectorEnv(**cfg)
...
export_policy_torch_old_api(
    ...
    "shared_policy",
    r"C:\Users\boris\BS_setup\bluesky-master\plugins\models_boris\Two_stage_AM_PPO_noise_Symetric_Kalman_Final.pt",
