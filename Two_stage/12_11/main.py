...
# ============================================================================
# CALLBACK CLASS - Must be in main.py for Ray workers to access it
# ============================================================================
...
def build_trainer(n_agents, stage=1, restore_path=None):
    ...
    if stage == 1:
        ...
        training_config = {
            "lr": 5e-4,  # Learning rate for imitation
            "train_batch_size": 4000,
            "minibatch_size": 128,
            "num_sgd_iter": 10,  # More updates per batch for imitation
            "clip_param": 0.3,  # PPO clipping
            "vf_loss_coeff": 0.01,  # Low value function coefficient
            "entropy_coeff": 0.0,  # No exploration in imitation
            "grad_clip": 1.0,
            "gamma": 0.99,
            "lambda_": 0.95,
...
if __name__ == "__main__":
    ...
    if run_stage1:
        ...
        for i in range(1, 51): # Run 50 iterations of cloning
            ...
            loss = "N/A"
            try:
                # Try to get from policy stats (where we store it)
                ...
                if hasattr(policy, 'loss_stats') and 'imitation_loss' in policy.loss_stats:
                    ...
                    # Record numeric loss
                    try:
                        stage1_loss_history.append(float(loss_val))
                    except Exception:
                        pass
                else:
                    # Try standard RLlib paths
                    loss = result.get('info', {}).get('learner', {}).get('shared_policy', {}).get('learner_stats', {}).get('total_loss', 'N/A')
                    try:
                        stage1_loss_history.append(float(loss))
                    except Exception:
                        pass
            ...
            print(f"Stage 1 - Iter {i}/50 | Imitation Loss: {loss}")
        ...
        # Set this as the restore point for Stage 2
        restored_from = stage1_path
        if RUN_STAGE_2:
