...
BASE_CHECKPOINT_DIR = os.path.join(script_dir, "models/sectorcr_ma_sac/stage1_weights")  # Stage 1 checkpoint
METRICS_DIR = os.path.join(script_dir, "metrics")

# --- CHOOSE WHICH CHECKPOINT TO EVALUATE ---
USE_BEST_CHECKPOINT = False  # Stage 1 only has one checkpoint, not best/final distinction
...
if USE_BEST_CHECKPOINT:
    ...
    if best_checkpoint:
        ...
        print(f"⚠️  No best checkpoint found, using Stage 1 checkpoint")
else:
    CHECKPOINT_DIR = BASE_CHECKPOINT_DIR
    print(f"📁 Using Stage 1 checkpoint (imitation-trained policy)")
