...
WARMUP_ITERATIONS = 10 # was 7  # Number of iterations to warm up critic with frozen policy
# --- STAGE 2: RL FINE-TUNING (PPO with standard loss) ---
TOTAL_ITERS = WARMUP_ITERATIONS + 100   #was 100.  set to 300 or something for ppo
...
def build_trainer(n_agents, stage=1, restore_path=None):
    ...
    if stage == 1:
        ...
        # Calculate timesteps for schedule (RLlib uses timesteps, not iterations)
        train_batch_size = 32000
        warmup_end_ts = WARMUP_ITERATIONS * train_batch_size  
        # Mid-training should be 2/3 of the way through (after warmup)
        mid_training_iter = WARMUP_ITERATIONS + int((TOTAL_ITERS - WARMUP_ITERATIONS) * 0.65)
        mid_training_ts = mid_training_iter * train_batch_size
        final_ts = TOTAL_ITERS * train_batch_size
        
        training_config = {
            # Learning rate schedule: Warmup → Main training → Final annealing
            "lr_schedule": [
                [0, 1e-5],              # Start: Warmup with low LR
                [warmup_end_ts, 5e-5],  # After warmup: Full training LR
                [mid_training_ts, 5e-5], # Keep stable through mid-training
                [final_ts, 5e-6],       # Final phase: Decay for convergence
            ],
            ...
            # Entropy schedule: Start low during warmup, increase for exploration, then anneal
            "entropy_coeff_schedule": [
                [0, 0.0],                # Warmup: No exploration bonus
                [warmup_end_ts, 0.01],   # Exploration kickstart after warmup
                [mid_training_ts, 0.01], # Maintain exploration
                [final_ts, 0.001],       # Final annealing for convergence
            ],
            "grad_clip": 0.5,
            "gamma": 0.99,
            "lambda_": 0.95,
