...
from bluesky_gym.envs.ma_env_two_stage_PPO import SectorEnv  # Use two_stage environment
...
# Set to True to use Stage 2 checkpoint, False to use Stage 1 checkpoint
USE_STAGE2_CHECKPOINT = True

# For Stage 1: Choose between best weights or last iteration
USE_BEST_STAGE1_WEIGHTS = False

# Determine which checkpoint to use based on the boolean flags
if USE_STAGE2_CHECKPOINT:
    CHECKPOINT_DIR = os.path.join(script_dir, "models/sectorcr_ma_sac/best_iter_00096")
    print(f"🔥 Using Stage 2 (RL Fine-Tuned) {CHECKPOINT_DIR}")
else:
    if USE_BEST_STAGE1_WEIGHTS:
        CHECKPOINT_DIR = os.path.join(script_dir, "models/sectorcr_ma_sac/stage1_best_weights")
        print(f"🌟 Using BEST Stage 1 (Imitation) weights: stage1_best_weights")
    else:
        CHECKPOINT_DIR = os.path.join(script_dir, "models/sectorcr_ma_sac/stage1_weights")
        print(f"📁 Using LAST Stage 1 (Imitation) weights: stage1_weights")
